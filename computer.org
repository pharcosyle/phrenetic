...
(define-module (solitude)
  #:use-module ((gnu home) #:select (home-environment this-home-environment))
  ...
  #:use-module ((gnu home-services fontutils) #:select (home-fontconfig-service-type))
  ...
  #:use-module ((gnu services) #:select (service service-kind simple-service))
  #:use-module ((gnu packages) #:select (specification->package))
  ...
  #:use-module ((srfi srfi-1) #:select (first remove))
  ...
  #:use-module ((solitude-fontconfig-service) #:select (solitude-fontconfig-service-type))
...
(define home-environment-default-essential-services
  (@@ (gnu home) home-environment-default-essential-services))

(define emacs-variant emacs-native-comp)

;; (define emacs-variant
;;   (let ((inferior
;;          (inferior-for-channels
;;           (list (channel
;;                  (name 'flat)
;;                  (url "https://github.com/flatwhatson/guix-channel.git")
;;                  (commit "64cdf99252db28408009c2c177649034f414b991"))
;;                 (channel
;;                  (name 'guix)
...
(define solitude
  (home-environment
   ;; (packages (list htop))
   (packages (list ;; emacs-variant ; TODO add this back when I can do so without eating up all my inodes. Also delete bastille manifest
   ...
   (essential-services (remove (lambda (service)
                                 (eq? (service-kind service) home-fontconfig-service-type))
                               (home-environment-default-essential-services this-home-environment)))
   (services
    (list
     (service solitude-fontconfig-service-type
              (list (file-append (@ (gnu packages fonts) font-adobe-source-code-pro) "/share/fonts")
                    (file-append (@ (gnu packages fonts) font-google-noto) "/share/fonts")))
...
*** Fontconfig Service
TODO
- check this out for writing xml programmatically: [[https://www.gnu.org/software/guile/manual/guile.html#SXML][Guile Reference Manual - SXML]]
- should "share/fonts" be automatically appended? I feel like I didn't do that for a reason?
- add "profile/share/fonts" to the test-for-on-change thing? Or just "profile" to the passed-in config (or support the potential to do that)
  + make it passed in manually or automatic? If it's automatic and the "profile/share/fonts" folder doesn't exist will my code fail?
  + test contents of "profile/share/fonts" as well as the live-fonts-file
#+begin_src scheme :tangle (in-tangle-dir "solitude-fontconfig-service.scm") :noweb yes
(define-module (solitude-fontconfig-service)
  #:use-module ((gnu services) #:select (service-extension service-type))
  ;; #:use-module ((gnu services configuration) #:select (define-configuration))
  #:use-module ((gnu home-services) #:select (home-profile-service-type home-run-on-change-service-type))
  #:use-module ((gnu home-services files) #:select (home-files-service-type))
  #:use-module ((gnu packages fontutils) #:select (fontconfig))
  ;; TODO narrow down or remove
  #:use-module (guix gexp)

  ;; #:use-module ((srfi srfi-1) #:select (list?))
  )

;; (define-configuration solitude-fontconfig-configuration
;;   (font-directories
;;    (list '("~/.guix-home/profile"))
;;    "TODO can I remove the this docstring?"))

(define live-fonts-file-simple-name "solitude-live-fonts")
(define live-fonts-file-path (string-append "config/." live-fonts-file-simple-name))

(define (add-files-service config)
...
"
  ...
  <dir>~/.guix-extra-profiles/multiverse/multiverse/share/fonts</dir>
</fontconfig>"))
    (,live-fonts-file-path
     ,(apply mixed-text-file live-fonts-file-simple-name
             (apply append (map (lambda (x) `(,x "\n")) config))))))

(define (regenerate-font-cache-gexp _)
  `((,(string-append "files/" live-fonts-file-path)
     ,#~(system* #$(file-append fontconfig "/bin/fc-cache") "-fv"))))

(define-public solitude-fontconfig-service-type
  (service-type (name 'solitude-fontconfig)
                (extensions
                 (list (service-extension
                       ...
                       (service-extension
                        ...
                        (const (list fontconfig))))) ; `fontconfig' seems to be installed by default in the base system but Guix Home adds it like this so I suppose I will too.
                (default-value #f)))

;; (use-modules (gnu package))
;; (define-public asdfffy
;;   (package
;;    (name "asdfffy")))
#+end_src
**** fonts.conf Template
#+begin_src xml :tangle (in-tangle-dir "fonts.conf")
<?xml version="1.0"?>
<!DOCTYPE fontconfig SYSTEM "fonts.dtd">
<fontconfig>
</fontconfig>
...
;; TODO Probably getting rid of this but if I keep it make this a reference to emacs-variant / the emacs variant inferior
(specifications->manifest
 '("emacs-native-comp"))
#+end_src
** Channels
