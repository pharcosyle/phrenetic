...
(define-module (base-os)
  #:use-module ((gnu system) #:select (operating-system %base-packages))
...
(define-public (stateless os)
  (operating-system
    ...
    (services
     (append
      (operating-system-user-services os)
      ;; Stateless service should come after base/desktop services so state gets installed before them during activation.
      (list
  ...
  (with-extensions (list guile-gcrypt)
    (with-imported-modules (source-module-closure
      ...
      #~(begin
          ...
          (invoke #$(file-append coreutils "/bin/echo") "working!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!working")

          ;; (with-output-to-file "asdffy.txt"
          ;;   (lambda _
          ;;     (invoke #$(file-append util-linux+udev "/bin/mount"))
          ;;     ;; (display "asdffffffffffff")
          ;;     ))

          ;; (with-output-to-file "qwerty.txt"
          ;;   (lambda _
          ;;     (invoke #$(file-append tree "/bin/tree"))
          ;;     ;; (display "asdffffffffffff")
          ;;     ))


          ;; (define (populate noclobber)

          ;;   (define (stash-or-pop op)
          ;;     (when (eq? op 'pop)
          ;;       (for-each delete-file noclobber))
          ;;     (for-each (lambda (path)
          ;;                 (let ((paths `(,path ,(string-append path ".stash"))))
          ;;                   (apply rename-file (if (eq? op 'stash)
          ;;                                          paths (reverse paths)))))
          ;;               noclobber))

          ;;   (stash-or-pop 'stash)
          ;;   (populate-root-file-system "dummy" "")
          ;;   (stash-or-pop 'pop))

          ;; (populate '("/var/guix/profiles/system"
          ...
          ;; (define (install-state target file)
          ;;   (mkdir-p (dirname target))
          ;;   (symlink file target))
          ;; (install-state "/var/lib/random-seed" "/.persist/var/lib/random-seed")


          (copy-file "/.persist/etc/group" "/etc/group")
          (copy-file "/.persist/etc/passwd" "/etc/passwd")
          (copy-file "/.persist/etc/shadow" "/etc/shadow")


          (invoke #$(file-append coreutils "/bin/echo") "done!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!done")
...
#+end_src
*** Macbook
- TODO should depend on the linux-nonfree module but the applying the latter twice isn't idempotent presently and I already have it in my frostfire OS config
...
(define-public (macbook os)
  (operating-system
    (inherit os)
    (kernel-arguments (let ((kernel-args %default-kernel-arguments)
                            (blacklist "b43,b43legacy,ssb,bcm43xx,brcm80211,brcmfmac,brcmsmac,bcma"))
                        (cons* (string-append (car kernel-args) "," blacklist) (cdr kernel-args))))
    (kernel-loadable-modules (list broadcom-sta))
    (firmware (cons* broadcom-bt-firmware
                     (operating-system-firmware os)))))

(define-public macbook-keyboard
...
(define my-cool-mapped-devices
  (list (mapped-device
          (source (uuid "bdda56af-6ca0-4953-bc13-d5af8715e0e5"))
          (target "ciphered")
          (type luks-device-mapping))))

(define my-cool-persist
  (file-system
    (device (file-system-label "firmament"))
    (mount-point "/.persist")
    (type "btrfs")
    (options (format #f "subvol=~a,compress=zstd,autodefrag" 'persist))
    (needed-for-boot? #t)
    (dependencies my-cool-mapped-devices)))

(define-public (frostfire os)
  (operating-system
    (inherit os)
    (host-name "frostfire")
    (mapped-devices my-cool-mapped-devices)
    (file-systems
      (append
       (map (match-lambda
              ((subvol . mount-point)
               (file-system
                 ...
                 (needed-for-boot? (if (memq subvol '(guix-var log)) #t #f))
       ...
       (list my-cool-persist)
       (list
        (file-system
          (device "/.persist/etc/passwd")
          (mount-point "/etc/passwd")
          (type "none")
          (flags '(bind-mount))
          (mount? #f)
          ;; (needed-for-boot? #t)
          (dependencies (list my-cool-persist)))
        (file-system
          (device "/.persist/etc/group")
          (mount-point "/etc/group")
          (type "none")
          (flags '(bind-mount))
          (mount? #f)
          ;; (needed-for-boot? #t)
...
#+end_src
*** Frostfire OS
#+begin_src scheme :tangle (in-tangle-dir "frostfire-os.scm")
(define-module (frostfire-os)
  ...
  #:use-module ((macbook) #:select (macbook macbook-keyboard))
  #:use-module ((frostfire) #:select (frostfire)))

(frostfire (macbook (stateless (linux-nonfree (base-os macbook-keyboard)))))
#+end_src
...
(define-module (solitude)
  ...
  ;; #:use-module ((gnu home-services shellutils) #:select (home-bash-direnv-service-type)) ; TODO RDE and upstream Guix Home have incompatibly diverged for the moment, comment this and the `home-bash-direnv-service-type` usage line below back in when they're back in sync.
