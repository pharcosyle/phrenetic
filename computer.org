...
(define-public (base-os kbl)
  (operating-system
    (locale "en_US.utf8") ; TODO this is the default, maybe don't bother specifying
...
(define-public (boot target label kbl os)
  (operating-system
    (inherit os)
...
(define-public (disk-encryption source-uuid target-name os)
  (operating-system
    (inherit os)
    (mapped-devices
     (list (mapped-device
...
(define-public (stateless persist-dir os)
  (operating-system
    (inherit os)
...
(define-public (macbook os)
  (operating-system
    (inherit os)
    (kernel-arguments (let ((kernel-args %default-kernel-arguments) ; TODO probably make this `operating-system-user-kernel-arguments`
...
(define-module (frostfire)
  #:use-module ((gnu system) #:select (operating-system operating-system-file-systems operating-system-mapped-devices))
  #:use-module ((gnu system file-systems) #:select (file-system file-system-label))
  #:use-module ((ice-9 match) #:select (match-lambda)))

(define-public (frostfire host-name label subvolumes-spec os)
  (operating-system
    (inherit os)
    (host-name host-name)
    (file-systems
       ...
       (map (match-lambda
              ((subvol mount-point)
               (file-system
                 ...
                 (needed-for-boot? (if (memq subvol '(guix-var log persist)) #t #f))
                 (dependencies (operating-system-mapped-devices os))))) ; TODO make dependent on disk-encryption feature
...
(define-module (frostfire-os)
  ...
  #:use-module ((disk-encryption) #:select (disk-encryption))
