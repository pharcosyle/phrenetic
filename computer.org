...
(use-modules ((gnu system) #:select (operating-system %default-kernel-arguments %base-firmware %base-packages))
             ...
             ((nongnu packages linux) #:select (linux-firmware broadcom-bt-firmware broadcom-sta))
...
#+end_src
*** Theme :UI:theming:
#+begin_src elisp :tangle gen/doom/themes/doom-nuclear-theme.el :lexical t
