...
(define-module (ebullient system components)
  ...
  #:use-module ((srfi srfi-1) #:select (any remove))
    ...
    (with-imported-modules (source-module-closure
      ...
      #~(begin
          ...
          (define* (populate #:key persist-dir #:allow-other-keys)
            ...
            ;; I'd like to do this during activation with the rest of the state but I can't get the activation service to run before account-service and I don't want to overwrite any changes it makes to these files so I can see and (manually) preserve them.
...
#+end_src
*** mtrack
- TODO Get the setting of keyboard-layout and xorg-config into et-cetera
  Note: The catchall for libinput is overriding a rule for Keyboards created by =gnu/services/xorg.scm= that sets the XKB stuff (layout, etc) set a in the guix xorg configuration
                   ...
                   (sddm-configuration
                    (xorg-configuration
                     (xorg-configuration
                      ...
                      (keyboard-layout (operating-system-keyboard-layout os))
                      (extra-config (list (with-input-from-file "ebullient/system/xorg.conf" read-string)))))))
          (operating-system-user-services os))))
#+end_src
**** Xorg config
...
#+end_src
*** Miscellany
nss-certs, SDDM, Gnome, backlight control, Nix, bluetooth
#+begin_src scheme
(define-system-comp (miscellany #:key desktop? laptop? nix? bluetooth?)
  ...
  (services
   (append
    (if desktop?
        (list ;; TODO bring this back once mtrack stuff is resolved
