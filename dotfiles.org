...
***** Dotfiles :path:
#+begin_src elisp
(load "~/projects/dotfiles/meta" 'noerror 'nomessage)
...
(let ((oob-dir (concat "~/projects/dotfiles/" oob-dir))) ; TODO currently `meta-get-dir' returns a path relative to the dotfiles dir. That might change but regardless don't hardcode the path here eventually.
...
;; TODO currently donnager is part of my dotfiles but if that isn't going to be the case I might like to make my dotfiles able to work in a "self-contained" fashion by providing a fallback in case it isn't available. To a github repo, or somehing.
(package! donnager
...
:meta-dir+: /projects
:END:
- REVIEW does this belong in the top-level org headlines?
  It doesn't really have anything to do with home environments (yet?) and could conceivably (though hopefully not?) contribute to the system config. Or maybe it should be considered effectively self-contained (like a channel), as if it were its own file (even though I may keep it in this file for convenience)
...
#+begin_src scheme
(define-module (ebullient home projects aws)
...
(define-module (ebullient home projects datomic)
  #:use-module (guix build-system copy)
  #:use-module (guix download)
  #:use-module (guix gexp)
  #:use-module (guix packages)
