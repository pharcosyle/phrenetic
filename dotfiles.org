...
(define (sway-wm os)
  (wm os
      #:package sway
      #:services ((feature-system-services-getter (feature-sway)) (rde-config))))
#+end_src
**** Custom session
#+begin_src scheme
(define* (custom-session os #:key session-name exe services)
  (wm os
      #:package
      (private-package
       (name (-> session-name
...
#+begin_src scheme
(define-system-comp (miscellany #:key desktop? laptop? nix? pipewire? bluetooth?)
  (packages
  ...
  (services
   (append
    ...
    (if laptop?
        ;; TODO This should really be a service contributed by Spartan's custom session. Doesn't matter now though: after removing EXWM move this into the Sway component or use the RDE backlight feature
        ...
        (if desktop?
            (-> $
                ...
                (custom-session #:session-name "Spartan WM"
                                #:exe "<<spartan-exe>>"
                                #:services (list (screen-locker-service slock)))
                hidpi-console-font)
            $))
      nonguix-substitutes
      (as-> $
...
** Components
:PROPERTIES:
:tangle-dir+: /components
:END:
*** asdf
:PROPERTIES:
:header-args+: :tangle (in-tangle-dir "asdf.scm")
:END:
#+begin_src scheme
(define-module (ebullient home components asdf)
  #:use-module (ebullient utils)
  ;; #:export ()
  )
#+end_src
*** Shepherd
:PROPERTIES:
:header-args+: :tangle (in-tangle-dir "shepherd.scm")
:END:
#+begin_src scheme
(define-module (ebullient home components shepherd)
  #:use-module (gnu services)
...
(define-module (ebullient home solitude)
  ...
  ;; For pipewire (at the time of this writing)
  ...
  ;; #:use-module (ebullient home components shepherd)
...
(define (remove-dbus-home-req services)
  (update-services*
   services
   (lambda (svc)
               ...
               (home-bash-configuration
                ...
                (bash-profile
                 `(,(slurp-file-gexp (local-file "bash_profile"))
                   ;; ,(file-append initialization) ; This shouldn't really be in the bash profile but I can't find a good way to run a script post-login.
                   )))))
     (filter service?
...
(define-public initialization
  (computed-file
   ...
   (with-imported-modules '((guix build utils))
     #~(begin
         (use-modules (guix build utils))
         (copy-file #$(local-file "script.sh") #$output)
         (substitute* #$output
           (("xkbcomp") #$(file-append xkbcomp "/bin/xkbcomp"))
           (("xkb_source=" all) (string-append all #$(local-file "current_setxkbmap_print.xkb")))
           (("xkb_custom=" all) (string-append all #$(local-file "xkb_custom" #:recursive? #t)))
           (("xinput") #$(file-append xinput "/bin/xinput"))
           (("xrandr") #$(file-append xrandr "/bin/xrandr")))))))
#+end_src
*** Script
...
(define desktop-el
  (computed-file
   ...
   (with-imported-modules '((guix build utils))
     #~(begin
         (use-modules (guix build utils))
         (copy-file #$(local-file "desktop.el") #$output)
...
(define startup-script
  (computed-file
   ...
   (with-imported-modules '((guix build utils))
     #~(begin
         (use-modules (guix build utils))
         (copy-file #$(local-file "startup-script.sh") #$output)
         (substitute* #$output
           (("initialization\\.sh") #$(file-append initialization))
           (("^xhost") #$(file-append xhost "/bin/xhost"))
           (("^xsettingsd") (string-append #$(file-append xsettingsd "/bin/xsettingsd")
                                           " --config=" #$(local-file "xsettingsd.conf")))
           (("^xset") #$(file-append xset "/bin/xset"))
           (("^xss-lock") #$(file-append xss-lock "/bin/xss-lock"))
           (("slock") "/run/setuid-programs/slock")
           (("dbus-launch") #$(file-append dbus "/bin/dbus-launch"))
           ;; (("emacs") #$(file-append emacs "/bin/emacs")) ; TODO When this was a package I was going to change this with input-rewriting, will that still work?
           (("emacs") "/home/pharcosyle/.guix-extra-profiles/bastille/bastille-6-link/bin/emacs") ; TODO hardcoded here temporarily
           ...
           (("desktop\\.el") #$desktop-el))
         (patch-shebang #$output (list #$(file-append bash "/bin"))) ; TODO `bash-minimal' might be okay/preferable here
         (chmod #$output #o555)))))
  ...
  (private-package
   ...
   (arguments
    `(#:modules ((guix build utils))
      ...
      ,#~(begin
           (use-modules (guix build utils))
           (let ((exe (string-append #$output "/bin/<<spartan-exe>>")))
             (mkdir-p (dirname exe))
             (symlink #$startup-script exe))
           #t)))))
#+end_src
*** Startup script
