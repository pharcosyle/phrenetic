...
(define (he-login-shell-service-type shell)
  (service-type
   ...
   (extensions
    (service-extension
     (case shell
       ((#:bash) home-bash-service-type)
       ;; ((#:zsh) home-zsh-service-type)
       )
     identity))
   (compose identity)
   (extend (partial login-shell-extensions shell))
   (default-value (kvm))))

(define (login-shell-extensions shell initial extensions)
  (let* ((config (cons* initial extensions))
         (profile (append-map (rpartial #:profile) config)))
    (case shell
      ((#:bash)
       (home-bash-extension
