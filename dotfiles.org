...
*** Basic info
#+begin_src scheme
(define* (basic-info os #:key host-name timezone locale)
  (operating-system
    (inherit os)
    (host-name host-name)
    (timezone timezone)
...
**** Substitutes
#+begin_src scheme
(define (with-linux-nonfree-substitutes services)
...
*** Disk encryption
#+begin_src scheme
(define* (encrypted-device #:key source-uuid target-name)
  (mapped-device
   (source (uuid source-uuid))
   (target target-name)
   (type luks-device-mapping)))

(define* (disk-encryption os #:key encrypted-device)
  (operating-system
    (inherit os)
    (mapped-devices
...
#+end_src
*** Stateless
The stateless service should come after base/desktop services (potentially others as well) so its state gets installed before theirs during activation. To this end the ~stateless-service-type~ is appended, not prepended, to operating system services and users of this component should include it at the end of their component chain.
#+begin_src scheme
