...
#+PROPERTY: tangle-dir ebullient-prism
...
mkdir -p ~/.config/guix
cp ~/projects/dotfiles/ebullient-prism/home/channels.scm ~/.config/guix
guix pull
hash guix
...
cd ~/projects/dotfiles/ebullient-prism
...
(define-module (base-os)
  ...
  #:use-module ((ebullient-prism packages xf86-input-mtrack) :select (xf86-input-mtrack))
...
(define-module (hidpi-console-font)
  #:use-module ((gnu system) #:select (operating-system operating-system-user-services))
  #:use-module ((gnu services) #:select (modify-services))
...
(define-module (frostfire-os)
  ...
  #:use-module ((nix) #:select (nix))
  #:use-module ((hidpi-console-font) #:select (hidpi-console-font))
...
sudo guix system --load-path=. reconfigure system/frostfire.scm
#+end_src
* Home
:PROPERTIES:
:tangle-dir+: home
...
(define-module (spartan core)
  ...
  ;; #:use-module ((ebullient-prism packages dracula-theme) #:select (dracula-theme))
  )
...
(define-module (ebullient-prism packages dracula-theme)
...
(define-module (ebullient-prism packages xf86-input-mtrack)
  #:use-module ((guix packages) #:select (package origin base32))
  #:use-module ((guix git-download) #:select (git-version git-fetch git-reference))
