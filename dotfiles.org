...
*** Custom Session
#+begin_src scheme
(define-system-comp (custom-session #:key session-name exe system-services)
  (packages
   (cons* (private-package
           (name (-> session-name
                     string-downcase
                     (string-tokenize (char-set-complement char-set:whitespace))
                     (string-join "-")))
...
(define-module (ebullient system base-os)
  #:use-module ((gnu services xorg) #:select (screen-locker-service))
  #:use-module ((gnu packages suckless) #:select (slock))
                ...
                (as-> $
                  (let ((session-exe "<<spartan-exe>>"))
                    (-> $
                        ...
                        (custom-session #:session-name "Spartan WM"
                                        #:exe session-exe
                                        #:system-services (list (screen-locker-service slock))))))
                (as-> $
...
;; TODO I don't think creating a package is really the right way to do this: I don't need or want to add any files to the profile, just to the store.
  ...
  (private-package
   ...
   (native-inputs
    `(("script.sh" ,(local-file "script.sh"))
      ("current_setxkbmap_print.xkb" ,(local-file "current_setxkbmap_print.xkb"))
      ("xkb_custom" ,(local-file "xkb_custom" #:recursive? #t))))
   (inputs
    (list xkbcomp xinput xrandr))
   (build-system trivial-build-system)
   (arguments
    `(#:modules ((guix build utils))
      #:builder
      ,#~(begin
           (use-modules (guix build utils))
           (substitute* (let ((newfile (string-append #$output "/script.sh")))
                          (mkdir #$output)
                          (copy-file #$(this-package-native-input "script.sh") newfile)
                          newfile)
             (("xkbcomp") (string-append #$(this-package-input "xkbcomp")
                                         "/bin/xkbcomp"))
             (("xkb_source=" all) (string-append all #$(this-package-native-input "current_setxkbmap_print.xkb")))
             (("xkb_custom=" all) (string-append all #$(this-package-native-input "xkb_custom")))
             (("xinput") (string-append #$(this-package-input "xinput")
                                        "/bin/xinput"))
             (("xrandr") (string-append #$(this-package-input "xrandr") "/bin/xrandr")))
           #t)))))
#+end_src
*** Script
...
;; TODO I don't need or want to add any files to the profile, just to the store (except for the spartan exe). Having the other files be part of the packge might not be the right way to do this?
...
(define-public spartan
  (private-package
   ...
   (inputs
    (list bash ; TODO `bash-minimal' might be okay/preferable here
          xhost
          xset
          xss-lock
          xsettingsd
   ...
   (arguments
    `(#:modules ((guix build utils))
      ...
      ,#~(begin
           (use-modules (guix build utils))
           (let ((desktop-el-file (let ((newfile (string-append #$output "/desktop.el")))
                                    (mkdir #$output)
                                    (copy-file #$(this-package-native-input "desktop.el") newfile)
                                    newfile)))
             ...
             (substitute* desktop-el-file
               (("(dunstctl|dunst)" all) (case (string->symbol all)
                                           ((dunst) (string-append #$(this-package-input "dunst")
                                                                   "/bin/dunst -config "
                                                                   #$(this-package-native-input "dunstrc")))
                                           ((dunstctl) (string-append #$(this-package-input "dunst")
                                                                      "/bin/dunstctl"))))
               (("nm-applet") (string-append #$(this-package-input "network-manager-applet")
                                             "/bin/nm-applet"))
               (("pasystray") (string-append #$(this-package-input "pasystray")
                                             "/bin/pasystray"))
               ;; (("redshift-gtk") (string-append #$(this-package-input "redshift")
               ;;                                  "/bin/redshift-gtk"))
               (("udiskie") (string-append #$(this-package-input "udiskie")
                                           "/bin/udiskie")))
             (let ((exe (let ((newfile (string-append #$output "/bin/<<spartan-exe>>")))
                          (mkdir-p (dirname newfile))
                          (copy-file #$(this-package-native-input "startup-script.sh") newfile)
                          newfile)))
               (substitute* exe
                 (("^xhost") (string-append #$(this-package-input "xhost")
                                            "/bin/xhost"))
                 (("^xsettingsd") (string-append #$(this-package-input "xsettingsd")
                                                 "/bin/xsettingsd --config="
                                                 #$(this-package-native-input "xsettingsd.conf")))
                 (("^xset") (string-append #$(this-package-input "xset")
                                           "/bin/xset"))
                 (("^xss-lock") (string-append #$(this-package-input "xss-lock")
                                               "/bin/xss-lock"))
                 (("slock") "/run/setuid-programs/slock")
                 (("dbus-launch") (string-append #$(this-package-input "dbus")
                                                 "/bin/dbus-launch"))
                 ;; (("emacs") (string-append #$(this-package-input "emacs")
                 ;;                           "/bin/emacs"))
               ...
               (patch-shebang exe (list (string-append #$(this-package-input "bash") "/bin")))
               (chmod exe #o555)))
           #t)))))
#+end_src
*** Startup script
...
(define-public emacs-desktop-environment-temp
  (package
    ...
    (source #f)
    (propagated-inputs
     (list brightnessctl playerctl scrot))
    (build-system trivial-build-system)
    (arguments
     `(#:modules ((guix build utils))
       #:builder
       ,#~(begin
            (use-modules (guix build utils))
            (mkdir-p (string-append #$output "/qwer"))
            (with-output-to-file (string-append #$output "/qwer/qwer.txt")
              (lambda _
                (display "qwerrrrrrr")))
            #t)))
    (synopsis #f)
  ...
  (let ((commit "124b80f3913e44cc7fdc1ac36a639252f7c4bf1b")
    ...
    (package
      ...
      (native-inputs
       (list pkg-config))
      (inputs
       (list xorg-server xorgproto xproto mtdev))
...
(define-public wireplumber
  (package
    ...
    (inputs
     (list dbus glib lua pipewire-0.3))
