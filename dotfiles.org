    ...
    (append
     ...
     (service home-zsh-service-type
              (home-zsh-configuration
               (zshrc
                ;; TODO trying out Andrew Tropin's zsh configuration verbatim, eventually I'll merge these two files
                (list (local-file "../../zsh/zshrc")
                      (local-file "zshrc")))))
...
guix home --load-path=. --expression='(@ (ebullient home solitude) he)' reconfigure
#+end_src
** Shells
*** Bash profile
:PROPERTIES:
...
#+end_src
#+begin_src scheme :tangle (in-tangle-dir "frostfire-channels-lock.scm")
(list (channel
        (name 'guix)
        ...
        (introduction
          (make-channel-introduction
            ...
            (openpgp-fingerprint
              "2A39 3FFF 68F4 EF7A 3D29  12AF 6F51 20A0 22FB B2D5")))))
#+end_src
#+begin_src scheme :tangle (in-tangle-dir "solitude-channels-lock.scm")
(list (channel
        (name 'guix)
        (url "https://git.savannah.gnu.org/git/guix.git")
        (branch "master")
...
#+end_src
#+begin_src sh :results output silent :dir base-tangle-dir :async
guix time-machine --load-path=. --channels=solitude-channels.scm -- describe --format=channels > solitude-channels-lock.scm
#+end_src
#+begin_src sh :results output silent :dir base-tangle-dir :async
guix pull --channels=solitude-channels-lock.scm
#+end_src
#+begin_src sh :results output silent :dir base-tangle-dir :async
guix time-machine --load-path=. --channels=frostfire-channels.scm -- describe --format=channels > frostfire-channels-lock.scm
#+end_src
#+begin_src sh :results output silent :dir base-tangle-dir :async
sudo guix time-machine --channels=solitude-channels-lock.scm -- system --load-path=. --expression='(@ (ebullient system hosts frostfire) os)' reconfigure
#+end_src
* Packages
:PROPERTIES:
:tangle-dir+: /ebullient/packages
:END:
** Emacs 29 with pgtk + native comp
