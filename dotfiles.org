  ...
  (lambda (x)
    (syntax-case x ()
      ((_ services
          ...
          dm-configuration
          dm-config-xorg
          dm-config-xorg-accessor
          field ...)
       (with-syntax ((cx (datum->syntax x 'cx)))
         #'(modify-services services
             (dm-service-type config =>
                              (dm-configuration
                               (inherit config)
                               (dm-config-xorg
                                (let ((cx (dm-config-xorg-accessor config)))
                                  (xorg-configuration
                                   (inherit cx)
                                   field ...)))))))))))
#+end_src
...
(define (auto-login-for-vt auto-logins vt*)
  (first
   (filter (lambda* (_ #:key vt #:allow-other-keys)
             (equals? vt vt*))
           auto-logins)))
...
(define (slim-config vt display auto-logins)
  (let ((config (slim-configuration
    ...
    (if auto-login
        (apply (lambda* (user-name #:key session #:allow-other-keys)
                 (slim-configuration
                  (inherit config)
...
**** Xorg modifier
#+begin_src scheme
(define-syntax-rule (modify-slim-xorg services field ...)
  (modify-xorg services
               slim-service-type
               slim-configuration
               xorg-configuration
...
#+begin_src scheme
(define-system-comp (xorg #:key xorg-modifier)
  (services
   (xorg-modifier (operating-system-user-services os)
                  (keyboard-layout (operating-system-keyboard-layout os))
                  (extra-config (cons* (string-appnend
                                        (with-input-from-file "ebullient/system/xorg.conf" read-string)
                                        "\n")
                                       (xorg-configuration-extra-config cx))))))
#+end_src
**** Conf
...
*** Mtrack
#+begin_src scheme
(define-system-comp (mtrack #:key xorg-modifier)
  (packages (cons* xf86-input-mtrack
                   (operating-system-packages os)))
  (services
   (xorg-modifier (operating-system-user-services os)
                  (modules (cons* xf86-input-mtrack
                                  (xorg-configuration-modules cx)))
                  (extra-config (cons* (string-appnend
...
(define* (create-os #:key
  ...
  (-> barebones-os
      ...
      linux-nonfree
      (users #:who `((,primary-user-name "<<user-full-name>>" #:admin? #t)
                    ...
                    (-> $
                        (slim #:vts '(7 8 9)
                              #:auto-logins (if disk-encryption?
                                                `((,primary-user-name #:session ,session-exe #:vt 7))
                                                '()))
                        (custom-session #:package-name "spartan-wm"
                                        #:desktop-name "Spartan WM"
                                        #:executable session-exe))))
                (as-> $
                  (let ((xorg-modifier slim-xorg-modifier))
                    (-> $
                        (xorg #:xorg-modifier xorg-modifier)
                        (mtrack #:xorg-modifier xorg-modifier))))
                hidpi-console-font)
            $))
      nonguix-substitutes
      (as-> $
      ...
      (as-> $
        (let* ((persist-dir "/.persist")
          ...
          (-> $
              ...
              (as-> $
                (if disk-encryption?
                    (disk-encryption $ #:source-uuid "<<frostfire-luks-uuid>>"
                                     #:target "ciphered"
...
#+end_src
** Pipewire
- REVIEW Using a patch for a newer pipewire because it's available and why not, remove this when updated in Guix.
