...
| =:db/id=         | =:user/full-name=      | =:user/login-shell= | =:user/sessions=             |
|------------------+------------------------+---------------------+------------------------------|
| :id.user/primary | Krzysztof Baranowski   | :shell/default      | :session/gnome :session/sway |
| :id.user/aux     | Phil Coulson           | :shell/default      | :session/gnome :session/sway |
| :id.user/guest   | Speak Friend and Enter | :shell/default      | :session/gnome               |
...
(define (get-home-channels #:rest cs)
  (apply get-channels (cons* rde cs)))
#+end_src
*** Definitions
#+begin_src scheme
(define rde
