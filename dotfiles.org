...
- TODO parameterize colors
  With gexps or noweb? Might be overkill but if I end up wanting the color values shared with other system components (like my window manager) it could make sense. I wouldn't have to manually check for =doom-one= updates either.
