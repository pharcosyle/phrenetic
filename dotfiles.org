...
#+begin_src scheme
(define (xorg-config os)
  (xorg-configuration
   (keyboard-layout (operating-system-keyboard-layout os))
   (extra-config (list (with-input-from-file "ebullient/system/xorg-keyboards.conf" read-string)))))
...
(define (slim-config vt display auto-logins)
  (let ((config (slim-configuration
                 (xorg-configuration (xorg-config os))
...
#+end_src
**** Xorg config
#+begin_src conf :tangle (in-tangle-dir "xorg-keyboards.conf")
Section "InputClass"
...
#+end_src
*** Mtrack
- TODO Parameterize the desktop manager service type and configuration type, don't hardcode SLiM's.
#+begin_src scheme
   ...
   (modify-services (operating-system-user-services os)
     (slim-service-type config =>
                        (slim-configuration
                         (inherit config)
                         (xorg-configuration
                          (let ((cx (slim-configuration-xorg config)))
                            (xorg-configuration
                             (inherit cx)
