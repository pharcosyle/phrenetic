...
* Utils
:PROPERTIES:
:header-args+: :tangle (meta-in-dir "ebullient/utils.el")
:END:
#+begin_src scheme
...
*** Serialize
#+begin_src elisp
(setq db--serialize-keyword #'identity)

(setq db--serializers
      (kvm
       :db.type/keyword db--serialize-keyword
       :db.type/string #'identity
       :db.type/boolean (lambda (expr)
                          (pcase expr
                            (t 'true)
                            (nil 'false)))))

(defalias 'db--serialize-attr db--serialize-keyword)

(defun db--serialize-value (db a v)
  (let ((type (db--attr-value-type db a)))
    (funcall (map-elt db--serializers type) v)))

(defun db--serialize-datom (db datom)
  (let ((e (db--dget datom :datom/e))
        (a (db--dget datom :datom/a))
        (v (db--dget datom :datom/v)))
    (list
     (db--serialize-value db :db/id e)
     (db--serialize-attr a)
     (db--serialize-value db a v))))
