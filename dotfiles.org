...
* Meta
:PROPERTIES:
:header-args+: :tangle (in-tangle-dir "meta.el")
:END:
#+begin_src elisp
(require 'dash)
...
** DB
#+begin_src elisp
(setq meta--db-library
      (expand-file-name "db/org-api.el" meta--base-dir))

(load (file-name-directory (buffer-file-name)) nil 'nomessage)
#+end_src
* Common
:PROPERTIES:
...
| =:db/id=         | =:user/full-name=      |
|------------------+------------------------|
| :id.user/primary | Krzysztof Baranowski   |
| :id.user/aux     | Phil Coulson           |
| :id.user/guest   | Speak Friend and Enter |
...
| =:db/id=         | =:user/username= |
|------------------+------------------|
| :id.user/primary | pharcosyle       |
| :id.user/aux     | pcoulson         |
...
(defun db--parse-string (db a raw-v)
...
(defun db--parse-entity (db raw-entity)
  (map-apply (lambda (raw-a raw-v)
               (let ((a (db--parse-string db :db/id raw-a)))
                 (cons a
                       (db--parse-string db a raw-v))))
             raw-entity))
#+end_src
*** Org tables
#+begin_src elisp
...
(defun db--tables-into-db (db tables)
  (->> tables
       (-mapcat #'db--parse-org-table)
       (-reduce-from #'db--raw-entity-into-db db)))

(defun db--load (table-refs)
  (db--tables-into-db
   (-mapcat #'db--entity->datoms db--base-schema)
