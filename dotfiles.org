...
(define-module (ebullient home components)
  #:use-module (guix gexp)
  #:use-module ((guix monads) #:select (with-monad return))
  ...
  #:use-module (ebullient utils)
  #:export (fontconfig-service-type
            pipewire-services
            sway-services
...
*** Fontconfig
#+begin_src scheme
(define (fonts-conf font-dirs)
  (append
   '("<?xml version='1.0'?>
<!DOCTYPE fontconfig SYSTEM 'fonts.dtd'>
<fontconfig>")
   (append-map (lambda (font-dir)
                 (list "<dir>" font-dir "</dir>"))
               font-dirs)
   '("</fontconfig>")))

(define (add-fontconfig-config-file font-dirs)
  `(("config/fontconfig/fonts.conf"
     ,(apply mixed-text-file
             "fonts.conf"
             (fonts-conf font-dirs)))))

(define font-watch-dir
  ".font-watch")
...
(define (watch-entry font-dirs)
  (with-monad %store-monad
    (return `((,font-watch-dir ,(directory-union "fonts" font-dirs))))))

(define fontconfig-service-type
  (service-type (name 'home-fontconfig)
                ...
                (compose concatenate)
                (extend (lambda (initial more)
                          (map (lambda (font)
                                 (file-append font "/share/fonts"))
                               (append initial more))))
                (default-value '())))
#+end_src
*** Pipewire
    ...
    (replace-services
     ...
     (lambda _
       (service fontconfig-service-type
                (list (specification->package "font-adobe-source-code-pro") ; TODO (propagated) dependency of Doom config, move it somewhere when I figure out how I'm doing Emacs/Doom guix configuration.
   ...
   (services
    (append
     ...
     (list
      ...
      ;; REVIEW Examples from RDE I may want to make use of.
...
(setq doom-theme 'doom-nuclear
      doom-font (font-spec :family "Source Code Pro Medium" :size 12))
#+end_src
...
(define (replace-services services kind replace-fn)
  (update-list
   services
   (lambda (svc)
     (eq? (service-kind svc) kind))
