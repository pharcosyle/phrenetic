    ...
    (package
      ...
      (source
       (origin
         ...
         (patches
          (list (local-file "ligatures.patch")))))
      ...
      (arguments
       `(;; #:install-plan
         ...
         ,#~(modify-phases %standard-phases
              (add-after 'install 'symlink-scripts-to-bin
                (lambda _
                  (let ((bin (string-append #$output:bin "/bin")))
                    (mkdir-p bin)
                    (for-each (lambda (f)
                                (symlink f (string-append bin "/" (basename f))))
