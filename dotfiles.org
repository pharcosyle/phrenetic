...
(define solitude
  (home-environment
   (packages (list (specification->package "font-fira-code-6") ; TODO (propagated) dependency of Doom config, move it somewhere when I figure out how I'm doing Emacs/Doom guix configuration.
   ...
   (services
    (append
     ...
     (list
      ...
      ;; TODO probably gate this and figure out how not to hardcode the path to my Guix source
      ;; TODO not sure why this isn't working nor why andrew tropin would use the "source" form instead of the (currently commented out) more direct form. He might just not have thought this through or tested yet
      (simple-service 'emacs-vterm-zsh-configuration
                      home-zsh-service-type
                      (home-zsh-extension
                       (zshrc
                        ;; (list (slurp-file-gexp (local-file "/home/pharcosyle/.config/guix/current/share/guile/site/3.0/rde/features/zsh/vterm")))
                        (list #~(format #f "source ~a"
                                        #$(local-file "/home/pharcosyle/.config/guix/current/share/guile/site/3.0/rde/features/zsh/vterm" "setup-vterm"))))))
...
***** UI :UI:theming:
#+begin_src elisp
(let ((font (font-spec :family "Fira Code" :size 12)))
  (when (find-font font)
    (setq doom-font font)))
...
- TODO removing the ~dired-omit-mode~ hook causes dired buffers to start off marked as modified. I don't think this was happening until I upgraded to Emacs 29 so maybe this will resolve itself on a later Emacs upgrade.
#+begin_src elisp
(use-package! dired-x
  :defer t
  :init
...
Using ~:init~ to configure variables (as opposed to ~:config~) but hlissner does it this way for the Geiser package so I will too.
#+begin_src elisp
(use-package! geiser
  :defer t
  :init
...
(package! org-tanglesync :pin "af83a73ae542d5cb3c9d433cbf2ce1d4f4259117") ; TODO maybe make this part of a Sway / Window Manager feature or introduced by my Dotfiles project
...
;; TODO remove when Doom bumps the vertico module
;; ------ begin vertico bumps
(package! vertico
  :recipe (:host github :repo "minad/vertico"
           :files ("*.el" "extensions/*.el"))
  :pin "716b618791268d14a53a4aa13f56bba6ff3fb346")

(package! orderless :pin "92008e762b30cb445a2227e458cbb9a5e1b1d4e8")

(package! consult :pin "b33cb2ef098b0f212d035e2677afdd7f04340765")
...
(package! embark :pin "1842d6b182c4fe7b6aa4d06f4957fbeabaa96de3")
(package! embark-consult :pin "1842d6b182c4fe7b6aa4d06f4957fbeabaa96de3")

(package! marginalia :pin "f352bb76df3dc4c6c28c12cfd9f71003a491760b")
...
- TODO generate this file with gexps?
  Might be overkill but if I end up wanting the color values shared with other system components (like my window manager) it could make sense. I wouldn't have to manually check for =doom-one= updates either.
...
(define-public emacs-29-pgtk-native-comp
  (emacs-from-git
   ...
   #:git-commit "552d58d5b96eaad1c036800d8b4af10cb6f2f823"
   #:checksum "1xph27x953pgigb09dkjgfcb10ldlclbxwa0hmmawmxyyx3865bn"))
#+end_src
** Fira Code
#+begin_src scheme :tangle (in-tangle-dir "fonts.scm")
(define-module (ebullient packages fonts)
  #:use-module (guix packages)
  #:use-module (guix download)
  #:use-module ((gnu packages fonts) #:select (font-fira-code)))

(define-public font-fira-code-6
  (package
    (inherit font-fira-code)
    (name "font-fira-code-6")
    (version "6.2")
    (source
     (origin
       ;; TODO Ideally inherit everything in `origin' but the SHA
