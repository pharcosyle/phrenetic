...
(define-public old-bsta
  (private-package
   (name "old-bsta")
   (native-inputs
    `(("bsta" ,(local-file "/gnu/store/c58r6ambrw9cs00gnmr90yg3wm8x8lf6-broadcom-sta-6.30.223.271" #:recursive? #t))))
    ...
    '(#:modules ((guix build utils))
      ...
      (begin
        ...
        (let* ((out (assoc-ref %outputs "out"))
          ...
          (let ((newfile (string-append out "/lib")))
            (mkdir-p newfile)
            (copy-file (string-append (get-input "bsta") "/lib") newfile)))
        #t)))))
...
(define-system-comp (macbook #:key bluetooth?)
  ...
  (kernel-loadable-modules (cons* old-bsta
                                  ;; broadcom-sta
...
speaker.")
  ...
  #+end_src
** broadcom-sta
#+begin_src scheme :tangle (in-tangle-dir "asdf.scm")
(define-module (ebullient packages asdf)
  #:use-module (gnu packages)
  #:use-module (gnu packages base)
...
(define broadcom-sta-version (@@ (nongnu packages linux) broadcom-sta-version))
(define broadcom-sta-i686-source (@@ (nongnu packages linux) broadcom-sta-i686-source))
(define broadcom-sta-x86_64-source (@@ (nongnu packages linux) broadcom-sta-x86_64-source))
         ...
         (replace 'unpack
           (lambda* (#:key inputs #:allow-other-keys)
             (let ((source (assoc-ref inputs "broadcom-sta-source")))
               ;; (copy-file (string-append source "/lib") "./lib")
               ;; (copy-file source ".")
               ((@@ (guix build utils) copy-recursively) source "." #:follow-symlinks? #t)
               (display ((@@ (guix build gnu-build-system) scandir) "."))
