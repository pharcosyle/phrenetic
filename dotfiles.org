...
(setq dt--palette
      (-mapcat (-lambda ((name v))
                 (list (intern (concat ":" name)) v))
               palette))

(defun dt--color (c)
  (plist-get dt--palette c))
...
(defun dt--synthesize-theme (f)
  (-as->
   ...
   (-reduce-from
    ...
    $
    (-concat
     (-map (-lambda ((var color))
             (list (rx "(" (literal var) (+ whitespace) "'(")
                   (dt--color color)))
           (-partition 2 (list "fg" :fg
                               "red" :red
                               "orange" :orange
                               "green" :green
                               "yellow" :yelow
                               ...
                               "violet" :violet
                               "cyan" :cyan)))
     (-map (-lambda ((var replacement))
             (list (rx "(" (literal var))
                   replacement))
           (-partition 2 (list "highlight" (dt--color :highlight)
                               "builtin" 'blue
                               "comments" (dt--color :comments)
                               "constants" 'yellow
                               "functions" 'blue
                               "keywords" 'magenta
                               "methods" 'violet
                               ...
                               "numbers" 'violet)))
     (list
      `(,(rx "line-number-current-line" (+ anything) ":foreground")
        (dt--color :violet)))))))

;; REVIEW do this better (don't hardcode it?). Add a :path: tag to this org header if necessary.
(dt--synthesize-theme "~/.config/emacs/.local/straight/repos/themes/themes/doom-one-theme.el")
#+end_src
**** File
