...
** Components
*** Barebones OS
#+begin_src scheme :tangle (in-tangle-dir "barebones-os.scm")
(define-module (ebullient system barebones-os)
  #:use-module ((gnu system) #:select (operating-system))
  #:use-module ((gnu system file-systems) #:select (%base-file-systems)))

(define-public barebones-os
  (operating-system
    (host-name #f)
    (timezone #f)
    (bootloader #f)
...
#+end_src
*** Basic Info
#+begin_src scheme :tangle (in-tangle-dir "basic-info.scm")
(define-module (ebullient system basic-info)
  #:use-module ((gnu system) #:select (operating-system)))

(define-public (basic-info os #:key host-name timezone locale)
  (operating-system
    (inherit os)
    (host-name host-name)
    (timezone timezone)
...
(define-module (ebullient system boot)
...
(define* (boot os #:key target label kbl)
  (operating-system
    (inherit os)
...
#+end_src
*** Initial Services
Other modules may wish to modify the list of operating system services so use this in the module chain early.
#+begin_src scheme :tangle (in-tangle-dir "initial-services.scm")
(define-module (ebullient system initial-services)
  #:use-module ((gnu system) #:select (operating-system operating-system-user-services))
  #:use-module ((gnu services base) #:select (%base-services))
  #:use-module ((gnu services desktop) #:select (%desktop-services)))

(define-public (initial-services os #:key desktop?)
  (operating-system
    (inherit os)
    (services
...
(define-module (ebullient system linux-nonfree)
  ...
  #:use-module ((ebullient system linux-nonfree-substitutes) #:select (with-linux-nonfree-substitutes))
...
(define* (linux-nonfree os #:key substitutes?)
  (operating-system
    ...
    (services (let ((user-services (operating-system-user-services os)))
                (if substitutes?
                    (with-linux-nonfree-substitutes user-services)
                    user-services)))))
#+end_src
**** Substitutes
#+begin_src scheme :tangle (in-tangle-dir "linux-nonfree-substitutes.scm")
(define-module (ebullient system linux-nonfree-substitutes)
...
(define-module (ebullient system disk-encryption)
...
(define-public (encrypted-device #:key source-uuid target-name)
  (mapped-device
   (source (uuid source-uuid))
   (target target-name)
   (type luks-device-mapping)))

(define-public (disk-encryption os #:key encrypted-device)
  (operating-system
    (inherit os)
    (mapped-devices
...
#+end_src
*** Stateless
The stateless service should come after base/desktop services (potentially others as well) so its state gets installed before theirs during activation. To this end the ~stateless-service-type~ is appended, not prepended, to operating system services and users of this module should include it at the end of their module chain.
#+begin_src scheme :tangle (in-tangle-dir "stateless.scm")
(define-module (ebullient system stateless)
  #:use-module ((gnu services) #:select (service))
  #:use-module ((gnu system) #:select (operating-system operating-system-initrd operating-system-file-systems operating-system-user-services))
  #:use-module ((ebullient system stateless-file-systems) #:select (stateless-file-systems))
  #:use-module ((ebullient system stateless-service) #:select (stateless-service-type))
  #:export (stateless))

(define* (stateless os #:key label persist-dir mountables dependencies  nix? bluetooth?)
  (operating-system
    (inherit os)
    ...
    (file-systems
     (append
      (stateless-file-systems label persist-dir mountables dependencies nix?)
      (operating-system-file-systems os)))
    (services
     (append
      (operating-system-user-services os)
...
#+end_src
**** File Systems
#+begin_src scheme :tangle (in-tangle-dir "stateless-file-systems.scm")
(define-module (ebullient system stateless-file-systems)
  #:use-module ((gnu system file-systems) #:select (file-system file-system-label))
  #:use-module ((ice-9 match) #:select (match-lambda))
  #:export (stateless-file-systems))

(define* (stateless-file-systems label persist-dir mountables dependencies nix?)
  (map (match-lambda
         ((purpose subvol mount-point)
          (file-system
            (device (file-system-label label))
            (mount-point mount-point)
            (type "btrfs")
            (options (string-append "subvol=" subvol ",compress=zstd,autodefrag"))
            (needed-for-boot? (if (member purpose '(#:guix-var #:log #:persist)) #t #f))
            (dependencies (or dependencies '())))))
       (apply (lambda* (#:key root boot guix-store guix-var log home persist nix)
                (append
                 `((#:root ,root "/")
                   (#:boot ,boot "/boot")
                   (#:guix-store ,guix-store "/gnu/store")
                   (#:guix-var ,guix-var "/var/guix")
                   (#:log ,log "/var/log")
                   (#:home ,home "/home")
                   (#:persist ,persist ,persist-dir))
                 ;; (if nix? `((#:nix ,nix "/nix") '()))
                 ))
              mountables)))
#+end_src
...
(define-module (ebullient system stateless-service)
...
#+end_src
*** Nix
#+begin_src scheme :tangle (in-tangle-dir "nix.scm")
(define-module (ebullient system nix)
  #:use-module ((gnu system) #:select (operating-system operating-system-user-services))
  #:use-module ((gnu services) #:select (service))
  #:use-module ((gnu services nix) #:select (nix-service-type)))

(define-public (nix os)
  (operating-system
    (inherit os)
    (services (cons* (service nix-service-type)
                     (operating-system-user-services os)))))
#+end_src
*** HiDPI Console Font
#+begin_src scheme :tangle (in-tangle-dir "hidpi-console-font.scm")
(define-module (ebullient system hidpi-console-font)
  #:use-module ((gnu system) #:select (operating-system operating-system-user-services))
  #:use-module ((gnu services) #:select (modify-services))
...
(define-public (hidpi-console-font os)
  (operating-system
    (inherit os)
...
#+begin_src scheme :tangle (in-tangle-dir "mtrack.scm")
(define-module (ebullient system mtrack)
  ...
  #:use-module ((ebullient packages xf86-input-mtrack) :select (xf86-input-mtrack))
  #:export (mtrack))

(define* (mtrack #:key kbl)
  (operating-system
    (inherit os)
    (packages
...
(define-module (ebullient system macbook)
...
#+end_src
*** Custom WM
#+begin_src scheme :tangle (in-tangle-dir "custom-wm.scm")
(define-module (ebullient system custom-wm)
  #:use-module ((gnu system) #:select (operating-system operating-system-packages))
  #:use-module ((ebullient system custom-wm-desktop-entry) #:select (custom-wm-desktop-entry)))

(define-public (custom-wm os #:key package-name desktop-name executable)
  (operating-system
    (inherit os)
    (packages (cons* (custom-wm-desktop-entry executable)
                     (operating-system-packages os)))))
#+end_src
**** Desktop Entry
#+begin_src scheme :tangle (in-tangle-dir "custom-wm-desktop-entry.scm")
(define-module (ebullient system custom-wm-desktop-entry)
  #:use-module ((guix packages) #:select (package))
  #:use-module ((guix build-system trivial) #:select (trivial-build-system))
  #:use-module ((ebullient utils) #:select (private-package)))

(define-public (custom-wm-desktop-entry package-name desktop-name executable)
  (package
    (inherit (private-package package-name))
...
#+end_src
*** Et Cetera
#+begin_src scheme :tangle (in-tangle-dir "et-cetera.scm")
(define-module (ebullient system et-cetera)
  #:use-module ((gnu system) #:select (operating-system operating-system-packages operating-system-user-services))
  #:use-module ((gnu services) #:select (service simple-service))
  #:use-module ((gnu services base) #:select (udev-service-type))
  #:use-module ((gnu services desktop) #:select (gnome-desktop-service-type bluetooth-service))
  ;; #:use-module ((gnu services xorg) #:select (xorg-configuration set-xorg-configuration))
  #:use-module ((gnu packages) #:select (specification->package))
  #:use-module ((gnu packages linux) #:select (brightnessctl))
  #:export (et-cetera))

(define* (et-cetera os #:key kbl bluetooth?)
  (operating-system
    (inherit os)
    (keyboard-layout kbl)
    (packages
...
(let ((bluetooth? #t)
      (nix? #t)
      (kbl macbook-kbl)
      (ciphered (encrypted-device #:source-uuid "bdda56af-6ca0-4953-bc13-d5af8715e0e5"
                                  #:target-name "ciphered")))
  (-> barebones-os
      (basic-info #:host-name "frostfire"
                  #:timezone "America/Los_Angeles"
                  #:locale "en_US.utf8")
      (boot #:target "/boot/efi"
            #:label "genesis"
            #:kbl kbl)
      (initial-services #:desktop? #t)
      (linux-nonfree #:substitutes? #t)
      (disk-encryption #:encrypted-device ciphered)
      (users #:who '(("<<user-name>>" "<<user-full-name>>" #:admin? #t)
                     ("pcoulson" "Phil Coulson" #:admin? #t)
                     ("gandalf" "Speak Friend and Enter" #:password "mellon"))
             #:bluetooth? bluetooth?)
      (as-> $
            (if nix? (nix $) $))
      hidpi-console-font
      ...
      (et-cetera #:kbl kbl
                 #:bluetooth? bluetooth?)
      (stateless #:label "firmament"
                 #:persist-dir "/.persist"
                 #:mountables '(#:root "root-blank"
                                #:boot "boot"
...
(define-module (solitude)
  ...
  #:use-module ((guix gexp) #:select (gexp file-append local-file))
...
(define-public initialization
  (package
    (inherit (private-package "initialization"))
    (native-inputs
...
(define-module (spartan core)
  ...
  ;; #:use-module ((ebullient packages dracula-theme) #:select (dracula-theme))
  #:use-module ((ebullient utils) #:select (private-package)))

(define-public spartan
  (package
    (inherit (private-package "spartan"))
    (build-system trivial-build-system)
    (native-inputs
...
;;              (guix git-download)
;;              ((guix build-system emacs) #:select (emacs-build-system))
...
(define-module (ebullient packages dracula-theme)
  ...
  #:use-module ((guix git-download) #:select (git-version git-fetch git-reference))
...
(define-public (private-package package-name)
  (package
    (name package-name)
    (version "0")
    (source #f)
