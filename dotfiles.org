...
(define-module (ebullient common channels)
...
(define-module (ebullient system channels)
  #:use-module (ebullient common channels)
  #:re-export (guix
               nonguix))
#+end_src
** Components
...
:tangle-dir+: /hosts
:END:
#+begin_src sh :var host_name="" hosts_dir=(get-tangle-dir)
sudo guix time-machine --channels="$hosts_dir"/$host_name-channels-lock -- system --load-path=. --expression="(@ (ebullient system hosts $host_name) host)" reconfigure
#+end_src
*** Frostfire
...
(define-module (ebullient home components)
  ...
  #:use-module (ebullient utils)
  #:export (zsh-direnv-service-type
            nix-service-type
            emacs-service-type
            doom-service-type
   ...
   (extensions
    (list
     (service-extension
      home-profile-service-type
      (lambda (config)
        ;; TODO Not ideal that emacs-service-type has to know that it might be being passed a path string (from a manifest) instead of a package
        (let ((emacs-package (assoc-ref config #:emacs-service/emacs)))
          (if (package? emacs-package)
              (list emacs-package) '()))))
     (service-extension
      home-environment-variables-service-type
      (lambda (config)
        `(("VISUAL" . ,(file-append (assoc-ref config #:emacs-service/emacs) "/bin/emacsclient"))
          ("EDITOR" . "$VISUAL"))))
     ;; TODO Does extending `home-zsh-service-type' introduce it if not present? If so I might need to create a wrapper around service-types so I can be selective about what service-extensions to even have for a home component.
     (service-extension
      home-zsh-service-type
      (lambda (config)
        (home-zsh-extension
         (zshrc
          (if (assoc-ref config #:zsh?)
              (list (local-file "../../zsh/vterm")) '())))))))
   (default-value
     (kvm #:emacs-service/emacs emacs-29-pgtk+native-comp))))
#+end_src
...
guix home --load-path=. --expression="(@ (ebullient home home-environments $he_name) he)" reconfigure
#+end_src
*** Solitude
:PROPERTIES:
:tangle-dir+: /home-environments
:END:
#+begin_src scheme :tangle (in-tangle-dir "solitude.scm") :noweb yes
(define-module (ebullient home home-environments solitude)
  #:use-module (guix gexp)
  #:use-module (gnu home)
  #:use-module (gnu home services)
  ...
  (home-environment
   ...
   (services
    (list
     ...
     (service emacs-service-type
              (kvm #:emacs-service/emacs (extra-profile-path "emacs-variant")))
...
#+call: gen-doom-theme() :results code :post unescape(*this*) :results_switches ":tangle (in-tangle-dir \"themes/doom-nuclear-theme.el\")"
