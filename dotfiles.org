...
(define-public (basic-info os host-name tz locale)
  (operating-system
    (inherit os)
    (host-name host-name)
    (timezone tz)
...
(define* (boot os target label #:optional kbl)
  (operating-system
    (inherit os)
...
#+begin_src scheme :tangle (in-tangle-dir "desktop-services.scm")
(define-module (ebullient system desktop-services)
...
(define-public (desktop-services os)
  (operating-system
    (inherit os)
    (services
...
(define* (linux-nonfree os #:key (substitutes? #t))
  (operating-system
    (inherit os)
    (kernel linux)
...
(define* (stateless os label persist-dir subvols #:optional dependencies #:key (nix? #t) (bluetooth? #t))
  (operating-system
    (inherit os)
    ...
    (file-systems
     (append
      (stateless-file-systems label persist-dir subvols dependencies nix?)
      (operating-system-file-systems os)))
    (services
     (append
      (operating-system-user-services os)
...
(define* (users os user-list #:key (bluetooth? #t))
  (operating-system
    (inherit os)
...
(define-public (custom-wm os package-name desktop-name executable)
  (operating-system
    (inherit os)
...
(define* (et-cetera os #:optional kbl #:key (bluetooth? #t))
  (operating-system
    (inherit os)
    (keyboard-layout kbl)
    (packages
...
(let ((kbl macbook-kbl)
  ...
  (-> barebones-os
      (basic-info "frostfire" "America/Los_Angeles" "en_US.utf8")
      (boot "/boot/efi" "genesis" kbl)
      desktop-services
      linux-nonfree
      (disk-encryption ciphered)
      (users '(("<<user-name>>" "<<user-full-name>>" #:admin? #t)
      ...
      (mtrack kbl)
      macbook
      (custom-wm "spartan-wm" "Spartan WM" "<<spartan-executable>>")
      (et-cetera kbl)
      (stateless "firmament"
                 "/.persist"
                 '(#:root "root-blank"
