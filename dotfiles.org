...
#+begin_src scheme
(define-system-comp (xorg #:key modify-xorg)
  (services
   (modify-xorg (operating-system-user-services os)
                (keyboard-layout (operating-system-keyboard-layout os))
...
*** Mtrack
#+begin_src scheme
(define-system-comp (mtrack #:key modify-xorg)
      ...
      (as-> $
        (if desktop?
            (-> $
                ...
                (as-> $
                  (let ((modify-xorg-fn modify-slim-xorg))
                    (-> $
                        (xorg #:modify-xorg modify-xorg-fn)
                        (mtrack #:modify-xorg modify-xorg-fn))))
                hidpi-console-font)
            $))
      nonguix-substitutes
      (as-> $
