...
| :feature/channel-deps | :db.type/symbol  | :db.cardinality/many |
| :user/channel-deps    | :db.type/symbol  | :db.cardinality/many |
| :user/full-name       | :db.type/string  | :db.cardinality/one  |
| :user/login-shell     | :db.type/keyword | :db.cardinality/one  |
| :user/sessions        | :db.type/keyword | :db.cardinality/many |
| :user/admin?          | :db.type/boolean | :db.cardinality/one  |
| :user/password        | :db.type/string  | :db.cardinality/one  |
| :user/username        | :db.type/string  | :db.cardinality/one  |
| :user/email           | :db.type/string  | :db.cardinality/one  |
| :color/hex            | :db.type/string  | :db.cardinality/one  |
...
|----------------------+-------------------------------------------------|
| :id.system/frostfire | :feature/nix :feature/desktop :feature/pipewire |

#+name: db/users-common
| =:db/id=         | =:user/channel-deps=  | =:user/full-name=      | =:user/login-shell= | =:user/sessions=             |
|------------------+-----------------------+------------------------+---------------------+------------------------------|
| :id.user/primary | 'nonguix 'flat        | Krzysztof Baranowski   | :shell/default      | :session/gnome :session/sway |
| :id.user/aux     | 'nonguix 'flat        | Phil Coulson           | :shell/default      | :session/gnome :session/sway |
| :id.user/guest   | 'nonguix              | Speak Friend and Enter | :shell/default      | :session/gnome               |
...
(define (get-system-channels system)
  (as-> system $
        (d:q $ '(#:system/features #:feature/channel-deps))
        (apply get-channels $)))
#+end_src
** Components
:PROPERTIES:
...
| =:db/id=               | =:feature/channel-deps= |
|------------------------+-------------------------|
| :feature/linux-nonfree | 'nonguix                |
...
(define* (base-os #:key
  ...
  (let* ((feature?
         ...
         (laptop? (and desktop?
                       laptop?))
         (macbook? (and laptop?
                        (feature? #:feature/macbook)))
         (pipewire? (and desktop?
                         (feature? #:feature/pipewire)))
         (bluetooth? (and desktop?
                          bluetooth?))
         (kbl (or kbl
                  (if macbook?
                      macbook-kbl #f)))
...
| =:db/id=             | =:system/features=                                                                  |
|----------------------+-------------------------------------------------------------------------------------|
| :id.system/frostfire | :feature/linux-nonfree :feature/disk-encryption :feature/stateless :feature/macbook |
...
(define-public os
  (base-os #:system #:id.system/frostfire
           #:host-name "frostfire"
           #:accounts '(#:id.user/primary
                        #:id.user/aux
                        #:id.user/guest)
           #:luks-uuid "<<frostfire-luks-uuid>>"
           ;; #:auto-login-account #:id.user/primary ; REVIEW Enable this later.
           #:console? #t
           #:laptop? #t
           #:bluetooth? #t))
#+end_src
...
(get-system-channels #:id.system/frostfire)
#+end_src
...
(define (get-home-channels user)
  (as-> user $
        (get (d:pull '(#:user/channel-deps) $) #:user/channel-deps)
        (cons 'rde $)
        (apply get-channels $)))
#+end_src
*** Definitions
...
(define-public he
  (base-he #:system #:id.system/frostfire
           #:account #:id.user/primary
           #:developer? #t))
#+end_src
**** Solitude old WIP
#+begin_src scheme :tangle (meta-in-dir "solitude-old-wip.scm")
(define-module (ebullient home he solitude)
  #:use-module (ebullient home base-he)
  #:export (he))
...
(get-home-channels #:id.user/primary)
#+end_src
**** Channels lock
...
(defun db--add (db datom)
  (if-let ((index (-find-index (-rpartial #'db--datom-match-e+a
                                          (db--dget datom :datom/e)
                                          (db--dget datom :datom/a))
                               db)))
      (if (equal :db.cardinality/many (db--val db (db--dget datom :datom/a) :db/cardinality))
          (-update-at index
                      (-partial #'-concat (db--dget datom :datom/v))
                      db)
        (->> db
             (-remove-at index)
             (cons datom)))
...
(defun db--parse-value (db a raw-v)
  (let* ((type (db--val db a :db/valueType))
         (parser (map-elt db--parsers type))
         (cardinality (db--val db a :db/cardinality)))
...
(defun db--tokenize-value (db a v)
  (let* ((type (db--val db a :db/valueType))
    ...
    (if (equal cardinality :db.cardinality/many)
        (-map (lambda (item)
                (funcall tokenizer item)))
      (funcall tokenizer raw-v))))
...
(setq db--marshallers
      (kvm
       :db.type/keyword db--marshall-keyword
       :db.type/string #'identity
...
(defun db--marshall-value (db a v)
  (let* ((type (db--val db a :db/valueType))
    ...
    (if (equal cardinality :db.cardinality/many)
        (-map (lambda (item)
                (funcall marshaller item)))
      (funcall marshaller raw-v))))
