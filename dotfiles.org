...
#+end_src
** Bash profile
#+begin_src sh :tangle (in-tangle-dir "bash_profile") :noweb yes
...
for i in $GUIX_EXTRA_PROFILES/*; do
  ...
  if [ -f "$profile"/etc/profile ]; then
    ...
    case $XDG_DATA_DIRS in
      ...
      ,*) export XDG_DATA_DIRS="$profile"/profile/share:$XDG_DATA_DIRS ;;
    esac
    case $MANPATH in
      ,*"$profile"/profile/share/man*) ;;
      ,*) export MANPATH="$profile"/profile/share/man:$MANPATH
    esac
    case $INFOPATH in
      ,*"$profile"/profile/share/info*) ;;
      ,*) export INFOPATH="$profile"/profile/share/info:$INFOPATH ;;
    esac
    case $XDG_CONFIG_DIRS in
      ,*"$profile"/profile/etc/xdg*) ;;
      ,*) export XDG_CONFIG_DIRS="$profile"/profile/etc/xdg:$XDG_CONFIG_DIRS ;;
    esac
    case $XCURSOR_PATH in
      ,*"$profile"/profile/share/icons*) ;;
      ,*) export XCURSOR_PATH="$profile"/profile/share/icons:$XCURSOR_PATH ;;
    esac
  fi
  unset profile
done
...
(add-to-list 'load-path "/home/pharcosyle/.guix-home/profile/share/emacs/site-lisp/exwm-0.25")
