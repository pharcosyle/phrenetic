...
- REVIEW Emacs 28 was released recently so Guix upstream emacs packages will probably be updated soon: review references to ~emacs-next~ and ~emacs-next-pgtk~, check flatwhatson's channel to see how it changes in response.
  - update: watch the =emacs-28= guix branch. I've updated this code in accordance with it up through d25054f. Still need to see how flatwhatson responds.
#+begin_src scheme
...
(define* (emacs-fully-loaded #:key pgtk?)
  (as-> (if pgtk?
            (with-emacs-latest emacs-next-pgtk)
            emacs) $
    (with-native-comp $)
...
#+end_src
****** Tramp path fix
The ~tramp-remote-path~ definition was moved in [[https://github.com/emacs-mirror/emacs/commit/695f6792f1524a446d276bf5c5e53bbb4c200909][Remove ;;;###tramp-autoload cookie from Tramp defcustoms (Bug#47063) · emacs-...]].
This fix should be upstreamed.
#+begin_src scheme
;; FIXME I haven't tested this /at all/. I didn't bother yet because I might be rewriting everything in lokke or clojure (and getting rid of mendicant utils).
;; In particular I don't know if equality between those two lists works.
(define (with-tramp-path-fix emacs)
  (package
    (inherit emacs)
    (source
     (origin
       ...
       (snippet
        (update-list
         ...
         (lambda (x)
           (and (list? x)
                (equal? '(substitute* "net/tramp-sh.el")
                        (take x 2))))
         (lambda (x)
           (append '(substitute* "net/tramp.el") (drop x 2)))))))))
#+end_src
...
;; At end: just maybe try doing this, slurp, any anything else impure as some sort of "coeffects"
(defn- file-exists? [f]
  (.exists (io/file f)))
                      ...
                      (let [text (slurp f)
                        ...
                        (merge
                         {:fex.package/url (let [fetcher (:fetcher parsed)]
                                             (condp some #{fetcher}
                                               #{'git} (:url parsed)
                                               #{'github 'gitlab} (let [repo (:reop parsed)]
                                                                    (forge-repo->url
                                                                     fetcher
                                                                     (:user-name repo)
                                                                     (:repo-name repo)))))}
