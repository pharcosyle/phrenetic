...
;; (s/def :epkg.req/name symbol?)
;; (s/def :epkg.req/tuple->name (s/conformer (fn [x]
;;                                             (if (list? x)
;;                                               (first x) x))))
;; (s/def :epkg/req (s/and :epkg.req/tuple->name
;;                         :epkg.req/name))
;; (s/def :epkg/reqs (s/coll-of :epkg/req :kind list?))



(s/def :epkg.dep/name symbol?)
(s/def :epkg.dep/tuple list?)
(s/def :epkg/dep (s/or :name :epkg.dep/name
                       :tuple :epkg.dep/tuple))
...
;; (s/def :elisp/car (s/conformer first))
(s/def :elisp/cdr (s/conformer rest)) ; TODO should this be `next`?
...
;; TODO can I make this optional? Or parameterize its inclusion (only check for it if read with edn/read-string)?
...
(s/def :elisp/t #{t})
(s/def :elisp/nil-or-non-nil any?)
...
(s/def :straight.recipe/source (s/or :one :straight.recipe-repo/name
                                     :many (coll-of :straight.recipe-repo/name)))
(s/def :straight.recipe/includes (s/or :one :straight.recipe/name
                                       :many (coll-of :straight.recipe/name)))
