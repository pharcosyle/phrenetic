...
(define (source-with-commit-and-hash p commit hash)
  (origin
    (inherit (package-source p))
    (uri (git-reference
          (inherit (origin-uri (package-source p)))
          (commit commit)))
    (sha256
     (base32
      hash))))
...
(define-public lokke
  (let ((commit "041e9a42b7579ac1af584e627c443f84962148a3")
    ...
    (package
      ...
      (source
       (source-with-commit-and-hash
        guile-xyz:lokke
        commit
        "1vx40r010gmz40lpamj862xvmslzr0krkpnvy44kij0l7frip5sb"))
      (arguments
              ...
              (add-after 'install 'move-installed-files
                (lambda _
                  (let* ((effective (target-guile-effective-version))
                    ...
                    ;; TODO move modules (scheme files), compiled go files, and extensions. Maybe patch the `load-extensions' calls too.
                    (display "Do stuff...")))))))))))
#+end_src
******* packaging doom WIP
...
#+end_src
******** third attempt
#+begin_src scheme :tangle (meta-in-dir "packaging-wip.scm") :comments link
(read-set! keywords 'prefix)

(use-modules (lokke ns))
(use-modules ((lokke core) :select (let if-let when-let case assoc merge))) ; TODO later: find out where the binding of e.g. `merge' is coming from (with and without the :select)
(require #(clojure.core :refer :all))

(defn update #(m k f & args)
  (apply update-in m (vector k) f args))
...
(defn select-keys #(m keyseq)
  (loop [ret (hash-map) keys (seq keyseq)]
    (if keys
      (let [k (first keys)
            v (get m k)]
        (recur
...
;; TODO hack. Maybe I can override stuff in the edn reader if it's not too tough.
;; - or find an elisp->clj/edn parser and do the parsing on the emacs side
;; - or maybe guile's parsing is a better match and I should use that
(defn icky #(x)
  (if (= x "0")
    "0x0" x))
...
(def archives (atom (hash-map)))

(defn add-archive! #(id methods)
  (swap! archives assoc id methods))

(defn get-recipe #(a & args)
  (apply (get-in (deref archives) (vector a 'get-recipe)) args))
...
(defn parse-melpa-recipe #(r*)
  (let [r (apply hash-map (rest r*))
        ...
        (merge
         (hash-map
          :recipe/flavor :flavor/melpa) ; TODO putting this here for completion for now, donno if I'll need it
         (let [fetcher (get r :fetcher)]
           (condp some (vector fetcher)
             (hash-set 'git 'hg) (hash-map
                                  :recipe/vc (case fetcher
                                               git :vc/git
                                               hg :vc/hg)
                                  :recipe/repo (get r :url))
             (hash-set 'github 'gitlab) (hash-map
                                         :recipe/vc :vc/git
                                         :recipe/host fetcher
                                         :recipe/repo (get r :repo))))
...
(add-archive!
 ...
 (hash-map
  'get-recipe (fn #(p)
                (when-let [s (slurp-extant (str "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/melpa" "/recipes/" p))]
                  (-> s
                      read-string
                      parse-melpa-recipe)))))

;; TODO just use `archives' for priority instead? Maybe not friendly to other users. Also I don't want a non-atom top-level form ideally maybe?
(def archive-priority (vector
                       :archive/melpa
                       :archive/elpa))
...
(defn parse-doom-package #(p*)
  (let [p (apply hash-map (rest p*))]
    (merge
     ...
     (when (some (hash-set (get p :type)) (vector 'local 'built-in))
       (hash-map
        :doom/type (case type
     ...
     (let [r (apply hash-map (get p :recipe))
           recipe
           (merge
            (when-let [type (get r :type)]
              (hash-map
               :recipe/vc (case type
                            git :vc/git
                            built-in :vc/built-in)))
            (when-let [flavor (get r :flavor)]
              (hash-map
     ...
     (select-and-rename-keys
      p (hash-map :pin :doom/pin))
     (when (or (get p :disable)
               (get p :ignore))
       (hash-map
        :package/deactivated? #t)))))
...
(defn parse-doom-packages #()
  (as-> (slurp "/home/pharcosyle/projects/phrenetic/doom-packaging/my-doom-packages.el") $
    ...
    (remove (fn #(p)
              (= (get p :package/name) "straight"))
            $)
    (remove (fn #(p)
              (get p :package/deactivated?))
            $)))
...
(defn parse-multi #(name dir)
  (when-let [s (slurp-extant (str dir "/" name "-pkg.el"))]
    (as-> (read-string s) $
      (nth $ 4)
...
(defn parse-single #(name dir)
  (when-let [s (slurp-extant (str dir "/" name ".el"))]
    (as-> s $
...
(defn package-deps #(name dir)
  (or (seq (parse-multi name dir))
      (parse-single name dir)))
...
(def straight-allow-recipe-inheritance? #t)

(def defaults
  (hash-map
   :recipe/protocol "https"
   ;; maybe?:
   ;; :files '("*.el" "*.el.in" "dir"
   ;;          "*.info" "*.texi" "*.texinfo"
...
;;                                   $))]
;;         (throw (ex-info "Package(s) with unsupported configuration" (hash-map :packages unsupported)))
;;         $)
#+end_src
********* elisp
