...
(s/def :epkg.dep/version (novalidate string?))
(s/def :epkg.dep/name+ver (s/cat :name :epkg.dep/name
                                 :_version (s/? (novalidate :epkg.dep/version))))
...
(s/def :epkg.multifile/reqs
  (s/and
   (s/cat :_define-package #{define-package}
          :_name (novalidate string?)
          :_version (novalidate :epkg.dep/version)
          :_docstring (s/? (novalidate string?))
          :requirements (s/? :elisp/quoted-form)
          :_extra (s/* (novalidate nil)))
...
;; TODO nb: ignoring :version-regexp and :old-names, I don't care about them, right (straight doesn't seem to)? And I'm excluding from even this validation spec stuff I don't care about, right? Or just maybe spec them but do a `s/select` that doesn't include them?
...
;; By serializing them "in the same run" as when I grab e.g `doom-packages`.
