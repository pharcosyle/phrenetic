...
(s/defop elisp-quoted-form [form-spec]
  (s/cat
   :quote #(= (symbol (str \')) %) ; "Artifact" of `edn/read-string`.
   :form form-spec))
(s/defop elisp-pair [car-spec cdr-spec]
  (s/cat :car car-spec
         :dot #{.}
         :cdr cdr-spec))
;; (s/def :elisp/list->car (s/conformer first)) ; TODO remove if not needed
(s/def :elisp/list->cdr (s/conformer next))
...
(s/def :epkg.multifile/reqs
  (s/cat :_define-package #{define-package}
         ...
         :requirements (s/? (elisp-quoted-form :epkg/deps))
         :_extra (s/* (nospec nil))))
...
(s/def :melpa.recipe.files/glob string?)
(s/def :melpa.recipe.files/subdirective (s/alt :subdir string?
                                               :exclude #{:exclude}))
(s/def :melpa.recipe.files/elements
  (s/cat
   :element (s/alt :glob :melpa.recipe.files/glob
                   :sublist (s/nest
                             (s/cat :subdirective :melpa.recipe.files/subdirective
                                    :elements (s/+ :melpa.recipe.files/elements))))))
...
(s/def :melpa.recipe.vc.git/commit (s/or :git/sha
                                         :git/ref))
(s/def :melpa.recipe/branch string?) ; TODO do better spec? I can be a git or hg branch, right?
...
;; TODO not doing testing for `list?` as in `elisp-list` here (or melpa recipe files). I might be getting rid of that anyway.
(s/def :straight.recipe/files
  (s/cat
   :element (s/alt :glob :melpa.recipe.files/glob
                   :pair (elisp-pair string? string?)
                   :sublist (s/nest
                             (s/cat :subdirective :melpa.recipe.files/subdirective
