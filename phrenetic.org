...
(defn get-recipe [a & args]
  (apply (get-in (deref archives) [a 'get-recipe]) args))
...
(add-archive!
 :archive/melpa
 {'get-recipe (fn [p]
                (when-let [s (slurp-safe (str "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/melpa" "/recipes/" p))]
                  (-> s
                      edn/read-string
                      parse-melpa-recipe)))})
...
(defn parse-multi [name dir]
  (when-let [s (slurp-safe (str dir "/" name "-pkg.el"))]
...
(defn parse-single [name dir]
  ["dash"]
  ;; (when-let [s (slurp-safe (str dir "/" name ".el"))]
  ;;   (as-> s $
