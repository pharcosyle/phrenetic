...
(use-modules ((lokke core) :select (let if-let when-let))) ; TODO could mendicant utils be causing the errors this is solving? Doubt it but check.
...
(defn icky #(x)
  (if (= x "0") ; TODO hack. Maybe I can override stuff in the edn reader if it's not too tough.
    "0x0" x))
...
(defn parse-melpa-recipe #(r)
  (as-> (read-string r) $
        ;; (hash-map
        ;;  :package/name (icky (str (first $)))
        ;;  :package/recipe (apply hash-map (rest $)))
        (apply hash-map (rest $))
        (merge
         (hash-map
          :recipe/flavor 'melpa ; TODO putting this here for completion for now, donno if I'll need it
          )
         (let [fetcher (get $ :fetcher)]
 ...
 (hash-map
  'get-recipe (fn #(p)
                  (when-let [s (slurp-extant (str "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/melpa" "/recipes/" p))]
                    (parse-melpa-recipe s)))))

;; TODO just use `archives' for priority instead? Maybe not friendly to other users.
...
(defn parse-doom-package #(p)
  (merge
   ...
   (as-> (apply hash-map (rest p)) $
     (select-and-rename-keys
      $
      (hash-map :type :doom/type
                :recipe :package/recipe
                :pin :doom/pin
                :disable :doom/disable
                :ignore :doom/ignore))
     (when (some (hash-set (get $ :doom/type)) (vector 'local 'built-in))
       (update :doom/type (fn #(type)
                              (cond type
                                'local :doom.type/local
                                'built-in :doom.type/built-in))))
     (update :package/recipe (partial apply hash-map)
             ...
             ;; TODO do renaming of keys
             ;; - rename :type to :recipe/vc, 'git and 'hg to :vc/git and :vc/hg
             ;; (fn #(r)
             ;;     (select-keys
             ;;      (apply hash-map r)
             ;;      #(:host :repo :branch :protocol :files :nonrecursive :pre-build :post-build ; TODO pre-build/post-build can take a bunch of forms, handle them or make them unsupported ; TODO protocol can be "ssh" in straight, handle or make unsupported.
             ;;        ;; :build :local-repo ; TODO might need these for donnager
             ;;        ;; :flavor ; TODO read `straight-expand-files-directive' and see if I need this / what I should do about :files. Also read the melpa readme about the rules it has for expanding its :files property
             ;;        )))
             )
     $)))
  ...
  (as-> (slurp "/home/pharcosyle/projects/phrenetic/doom-packaging/my-doom-packages.el") $
        ...
        (if-let [unsupported (seq (filter (fn #(p)
                                              (or (let [type (get p :doom/type)]
                                                  ...
                                                  (let [recipe-type (get-in p (vector :package/recipe :recipe/vc))]
                                                    (and recipe-type (not= recipe-type :vc/git)))
                                                  (get-in p (vector :package/recipe :recipe/protocol))
                                                  (get-in p (vector :package/recipe :recipe/source))
                                                  (get-in p (vector :package/recipe :recipe/fork))
                                                  (get-in p (vector :package/recipe :recipe/upstream))
                                                  (get-in p (vector :package/recipe :recipe/includes))))
                                          $))]
