...
:END:
- REVIEW Emacs 28 was released recently so Guix upstream emacs packages will probably be updated soon: review references to ~emacs-next~ and ~emacs-next-pgtk~, check flatwhatson's channel to see how it changes in response.
#+begin_src scheme
...
(define* (emacs-fully-loaded #:key pgtk?)
  (as-> (if pgtk?
    ...
    (with-zstd $)))
#+end_src
****** With Emacs 29 :version:
#+begin_src scheme
(define-public (with-emacs-29 emacs)
  (let ((commit "aeb25f9d3d12a18ef3881e23b32a34615355d4d0")
        (revision "0"))
    (package
      (inherit emacs)
      ...
      (source
       (origin
         ...
         (method git-fetch)
         (uri (git-reference
               (url "https://github.com/emacs-mirror/emacs") ; https://git.savannah.gnu.org/git/emacs.git is the canonical repo but it doesn't allow shallow fetches.
               (commit commit)))
         (sha256
          (base32
           "1mikbbibngk60klr1q7d543rbg5a1lz0rg2acvy125zjczvr7pww"))
         (file-name (git-file-name (package-name emacs) version)))))))
#+end_src
****** With native comp
#+begin_src scheme
(define-public (with-native-comp emacs)
  (emacs-with-native-comp emacs gcc-11 'full-aot))
#+end_src
****** With findable C source
#+begin_src scheme
...
#+end_src
****** With =zstd=
- REVIEW Just add a propagated input for now. Using ~wrap-program~ on the emacs executable would be better but it's a bit tricky (see the ~strip-double-wrap~ build phase in the emacs package definition) and I don't want to mess it up and introduce possibly subtle problems.
#+begin_src scheme
...
(define-module (ebullient home emacs-xyz)
  ...
  #:use-module ((guix licenses) #:prefix license:)
  #:use-module ((guix utils) #:select (substitute-keyword-arguments))
  #:use-module ((gnu packages emacs-xyz) #:select (emacs-dash emacs-tldr) #:prefix emacs-xyz:)
...
******* Lokke
TODO move me
- TODO [[(lokke-moving)][moving ouput files]]
  move modules (scheme files), compiled go files, and extensions. Maybe patch the `load-extensions' calls too.
- TODO maybe: clj files aren't compiled to =.go= files ahead of time, maybe do this
  Minor optimization but it impacts startup time after clearing =~/.cache/guile/ccache=
#+begin_src scheme :tangle (meta-in-dir "guile-xyz.scm")
(define-module (ebullient home guile-xyz)
  #:use-module (guix build-system gnu)
  #:use-module (guix gexp)
  #:use-module (guix git-download)
  #:use-module (guix packages)
  #:use-module ((guix utils) #:select (substitute-keyword-arguments))
  #:use-module ((gnu packages guile-xyz) #:select (lokke) #:prefix guile-xyz:))

(define-public lokke
  (let ((commit "041e9a42b7579ac1af584e627c443f84962148a3")
        (revision "0"))
    (package
      (inherit guile-xyz:lokke)
      (version (git-version "0.0.1" revision commit)) ; The guix package uses "0.0.0" for the version but the lokke version.scm says "0.0.1".
      (source
       (origin
         (method git-fetch)
         (uri (git-reference
               (url (git-reference-url (origin-uri (package-source guile-xyz:lokke))))
               (commit commit)))
         (sha256
          (base32
           "0ff4fw9fwm92wzrqxb56g0hdqi5kpb56l0ar2124dldcrpfa28cf"))))
      (arguments
       (substitute-keyword-arguments (package-arguments guile-xyz:lokke)
         ((#:imported-modules imported-modules %gnu-build-system-modules)
          `((guix build guile-build-system)
            ,@imported-modules))
         ((#:modules modules '((guix build gnu-build-system)
                               (guix build utils)))
          `(((guix build guile-build-system) #:select (target-guile-effective-version))
            ,@modules))
         ((#:phases phases '%standard-phases)
          #~(modify-phases #$phases
              (add-after 'install 'move-installed-files
                (lambda _
                  (let* ((effective (target-guile-effective-version))
                         (module-dir (string-append #$output "/share/guile/site/" effective))
                         (go-dir (string-append #$output "/lib/guile/" effective "/site-ccache"))
                         (extensions-dir (string-append #$output "/lib/guile/" effective "/extensions")))
                    (display "Do stuff..."))))))))))) ; (ref:lokke-moving)
#+end_src
******* packaging doom WIP
...
(s/defop novalidate [_]
  any?)


(s/def :elisp/t #{t})
(s/def :elisp/symbol symbol?)
(s/def :elisp/string string?)
(s/defop elisp-list [spec]
  (s/coll-of spec :kind list?))
;; "Artifact" of `edn/read-string`.
(s/def :elisp/quoted-form
  (s/cat
   :quote #(= (symbol (str \')) %)
   :form any?))
;; (s/def :elisp/car (s/conformer first)) ; TODO remove if not needed
(s/def :elisp/cdr (s/conformer next))
(s/def :elisp/plist->map (s/conformer (partial apply hash-map)))
(s/def :elisp/nil-or-non-nil any?) ; TODO think this (perhaps just the name) through more?


(s/def :epkg.dep/name :elisp/symbol)
(s/def :epkg.dep/name+ver (s/cat :name :epkg.dep/name
                                 :_version (s/? (novalidate :elisp/string))))
...
(s/def :epkg/deps (elisp-list :epkg/dep))
(s/def :epkg/deps->string-names
  (s/coll-of (s/and
...
;;  (s/and
;;   :epkg/deps
;;   :epkg/deps->string-names)
;;  test-deps)



;; (s/def ::f string?)
;; (s/def ::l (s/conformer (partial str "asdf")))
;; (s/def ::s (s/schema [::f ::l]))
;; (s/conform ::s {::f "Bugs" ::l "Bunny" ::x 10})

;; (s/def ::f string?)
;; (s/def ::l (s/conformer (partial str "asdf")))
;; (s/def ::s (s/schema {:f ::f :l ::l}))
;; (s/conform ::s {:f "Bugs" :l "Bunny" :x 10})

#_(defn parse-melpa-recipe [r*]
  (let [r (apply hash-map (rest r*))
        recipe
        (merge
         {:recipe/flavor :recipe.flavor/melpa} ; TODO putting this here for completion for now, donno if I'll need it
         (let [fetcher (:fetcher r)]
           (condp some #{fetcher}
             #{'git 'hg} {:recipe/vc (case fetcher
                                       git :recipe.vc/git
                                       hg :recipe.vc/hg)
                          :recipe/repo (:url r)}
             #{'github 'gitlab} {:recipe/vc :recipe.vc/git
                                 :recipe/host fetcher
                                 :recipe/repo (:repo r)}))
         (select-and-rename-keys
          r {:branch :recipe/branch
             :files :recipe/files}))]
    (when-not (empty? recipe)
      recipe)))
...
;;   (s/and
;;    :elisp/something
;;    (s/schema {:fetcher #{git hg github gitlab}})))

;; (s/valid? :melpa/rc test-melpa-recipe)
;; (s/explain-str :melpa/rc test-melpa-recipe)
;; (s/conform :melpa/rc test-melpa-recipe)

;; (s/def :parsed.melpa/rc
;;   (s/and
;;    :elisp/something
;;    ))
...
;; (defmacro match-seq [s]
;;   (let [ks (mapv keyword (repeatedly (count s) gensym))
;;         cat-kvs (mapcat vector ks (map hash-set s))]
;;     `(s/&
;;       (s/cat ~@cat-kvs)
;;       (s/conformer
;;        (juxt ~@ks)))))
...
;; (defmacro match-seq* [s]
;;   (let [key-preds (mapcat vector
;;                           (map keyword (repeatedly gensym))
;;                           (map hash-set s))]
;;     `(s/cat ~@key-preds)))
...
;; (s/defop match-seq [s]
;;   (s/& (s/+ any?) (partial = (seq s))))

(s/def :char/blank #{\space \tab})
(s/def :char/newline #{\newline})

(s/def :epkg.simple/req-header-name
  (s/& (s/+ any?) #(= (seq "Package-Requires:") %)))

;; (s/defop match-t [a]
;;   (s/and string?
;;          a))

;; (s/def ::asdf (match-t (s/conformer parse-double)))
...
;; (s/conform ::first-name "asdfa")


;; (s/def :text/lines (s/and
;;                     ;; (s/conformer seq)
;;                     (s/*
;;                      (s/alt
;;                       :line (s/cat (s/* any?))
;;                       ;; :line any?
;;                       ;; :newline (s/? :char/newline)
;;                       :newline :char/newline
;;                       ))
;;                     ;; (s/conformer (fn [[line _]] line))
;;                     ))

(require '[clojure.string :as str])

(s/def :text/lines (s/conformer str/split-lines))
...
;; (s/def ::asdf (s-filter :epkg.dep/name))
;; (s/conform ::asdf ["a" 'asdf])
;; (s/conform :text/lines "asdf\ndfdf\n")

;; TODO "elegance": maybe re-conform matches to strings here. Maybe make spec that wraps such string-regex operations with the necessary `seq`/`apply-str` at the beginning/end.
...
(s/def :epkg.simple/reqs
  (s/and
   ...
   (s/conformer #(->> % :deps (apply str) edn/read-string))
   :epkg/deps))
...
;; (defn epkg-reqs-files [name dir]
;;   (if (file-exists? (str dir "/" name "-pkg.el"))))

;; (some #{"/home/pharcosyle/projects/hickeyland/src/protostar/hickeyland.clj"}
;;       (map str (file-seq (io/file "/home/pharcosyle/projects/hickeyland"))))

;; (s/defop cofx [f]
;;   (s/conformer ))

;; (s/def :cofx/file-exists?
;;   (s/conformer (fn [f]))
;;   )



;; TODO just maybe don't underscore-"ignore" parts and have real specs (not just `any?`) for the other bits? I don't care about them but maybe I want to document the shape of the data?
(s/def :epkg.multifile/reqs
  (s/and
   (s/cat :_function #{define-package}
          :_name any?
          :_version any?
          :_docstring (s/? any?)
          :requirements (s/? :elisp/quoted-form)
          :_extra (s/* any?))
...
;; (defmulti epkg-type :epkg/type)
;; (defmethod epkg-type :epkg.type/simple [_]
;;   :epkg.simple/reqs)
;; (defmethod epkg-type :epkg.type/multifile [_]
;;   (s/conformer )
;;   :epkg.multifile/reqs)
;; (s/def ::epkg (s/multi-spec epkg-type :epkg/type))
...
(defn epkg-requirements [name dir]
  (let [deps
        (s/conform
         (s/and
          :epkg/reqs
...
;; (epkg-requirements
;;  "magit"
;;  "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/magit/lisp")
;; (println (package-deps "dash" "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/dash"))
;; (println (package-deps "helpful" "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/helpful"))
;; (println (package-deps "evil" "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/evil"))
;; (println (package-deps "drag-stuff" "/home/pharcosyle/.local/var/lib/doom-local/straight/repos/drag-stuff.el"))
...
(s/def :melpa.recipe.forge/repo string?)
;; nb: "commit" is used only once in all of the melpa recipes. Do doom/straight even consider it?
(s/def :melpa.recipe.vc.git/commit (s/or :git/sha
                                         :git/ref))
(s/def :melpa.recipe/branch string?) ; do better spec? I can be a git or hg branch, right?
(s/def :melpa.recipe/files list?) ; TODO read the melpa README and straight's expand-files-directive to suss this out. Call the spec ":melpa/files"?
;; nb: ignoring :version-regexp and :old-names, I don't care about them, right (straight doesn't seem to)? And I'm excluding from even this validation spec stuff I don't care about, right? Or just maybe spec them but do a `s/select` that doesn't include them?

(s/def :melpa.recipe/schema
  (s/schema [:melpa.recipe/name
             :melpa.recipe/fetcher
             :melpa.recipe.vc/url
             :melpa.recipe.forge/repo
             :melpa.recipe.vc.git/commit
             :melpa.recipe/branch
             :melpa.recipe/files]))

(defmulti melpa-recipe-fetcher-type :melpa.recipe/fetcher)

(defmethod melpa-recipe-fetcher-type 'git [_]
  (s/select :melpa.recipe/schema [:melpa.recipe.vc/url]))
(defmethod melpa-recipe-fetcher-type 'hg [_]
  (s/select :melpa.recipe/schema [:melpa.recipe.vc/url]))
(defmethod melpa-recipe-fetcher-type 'github [_]
  (s/select :melpa.recipe/schema [:melpa.recipe.forge/repo]))
(defmethod melpa-recipe-fetcher-type 'gitlab [_]
  (s/select :melpa.recipe/schema [:melpa.recipe.forge/repo]))

(s/def :melpa/recipe
  (s/and- ; TODO why did this have to be `and-` again? I know the test conform I was doing below fails without it.
   :melpa.recipe/schema
   (s/multi-spec melpa-recipe-fetcher-type :melpa.recipe/fetcher)))

;; (s/conform
;;  :melpa/recipe
;;  {:melpa.recipe/branch "aaa"
;;   :melpa.recipe.vc.git/commit "asdf"
;;   :melpa.recipe/fetcher 'git
;;   :melpa.recipe.vc/url "http..."
;;   ;; :epkg.dep/name 'what
;;   })


#_(let [r (apply hash-map (:recipe p))
      recipe
      (merge
       (when-let [type (:type r)]
         {:recipe/vc (case type
                       git :recipe.vc/git
                       built-in :recipe.vc/built-in)})
       (when-let [flavor (:flavor r)]
         {:recipe/flavor (case flavor
                           melpa :recipe.flavor/melpa)})
       (select-and-rename-keys
        r {:host :recipe/host
           :repo :recipe/repo
           :branch :recipe/branch
           :files :recipe/files
           :nonrecursive :recipe.straight/nonrecursive
           :build :recipe.straight/build
           :pre-build :recipe.straight/pre-build
           :post-build :recipe.straight/post-build
           :local-repo :recipe.straight/local-repo
           :protocol :recipe.straight/protocol
           :source :recipe.straight/source
           :fork :recipe.straight/fork
           :upstream :recipe.straight/upstream
           :includes :recipe.straight/includes}))]
  (when-not (empty? recipe)
    {:package/recipe recipe}))

#_(&key local-repo _files _flavor
      _build _pre-build _post-build _includes
      _type _repo _host _branch
      _remote _nonrecursive _fork _depth)

#_(defun straight-vc-git-keywords ()
...
(s/def :straight/username+repo
  (s/and
   (s/conformer seq)
   (s/cat :username (s/+ char?)
          :slash #{\/}
          :repo (s/+ char?))))
...
(s/def :straight.recipe/files list?) ; TODO read straight's straight-default-files-directive and  straight-expand-files-directive to suss this out
(s/def :straight.recipe/flavor #{melpa})
(s/def :straight.recipe/built any?) ; TODO. Had #{nil t}, it would be better to use :elisp/t but set specs take literal, not spec, values
...
(s/def :straight.recipe/source (s/or :one :straight.recipe-repo/name
                                     :many (elisp-list :straight.recipe-repo/name)))
(s/def :straight.recipe/includes (s/or :one :straight.recipe/name
                                       :many (elisp-list :straight.recipe/name)))
(s/def :straight.recipe/inherit :elisp/nil-or-non-nil)
;; TODO ick, the value straight.recipe.vc/repo can have is conditional on :host, which is a pattern clojure spec (I think) doesn't support
(s/def :straight.recipe.git/repo (s/or :u+r :straight/username+repo
...
(s/def :straight.recipe/schema
  (s/schema [:straight.recipe/name
             :straight.recipe/local-repo
             :straight.recipe/files
             :straight.recipe/flavor
             :straight.recipe/built
             :straight.recipe/pre-build
             :straight.recipe/post-build
             :straight.recipe/type
             :straight.recipe/source
             :straight.recipe/includes
             :straight.recipe/inherit
             :straight.recipe.git/repo
             :straight.recipe.git/host
             :straight.recipe.git/branch
             :straight.recipe.git/remote
             :straight.recipe.git/nonrecursive
             :straight.recipe.git/fork
             :straight.recipe.git/depth
             :straight.recipe.git/protocol
             :straight.recipe.git/upstream]))

(s/def :straight/recipe
  (s/and-
   :straight.recipe/schema
   ;; TODO do the repo-host constraint described here: https://github.com/raxod502/straight.el#git-backend
   ))


#_(defn parse-doom-package [p*]
  (let [p (apply hash-map (rest p*))]
    (merge
     {:package/name (icky (str (first p*)))}
     (let [type (:type p)]
       (when (some #{type} ['local 'built-in])
         {:doom.package/type (case type
                       local :doom.package.type/local
                       built-in :doom.package.type/built-in)}))
     ;; TODO read https://github.com/melpa/melpa#recipe-format
     ;; TODO read `straight-recipes-melpa-retrieve', particularly the big comment inside
     ;; TODO pre-build/post-build can take a bunch of forms, handle them or make them unsupported
     ;; TODO protocol can be "ssh" in straight, handle or make unsupported.
     ;; - update: I already have a check for it in my "error-reporting" thing below but I should make sure that's it's not an /invalid/ value ("https" is fine if its stated explicitly)
     ;; TODO might need :build and :local-repo for donnager
     ;; - update: local-repo is /also/ the dir name that straight assigns for a package (if it's not a full path). I can probably ignore this use case.
     ...
     ;; TODO at end: ensure an empty map isn't added / the map doesn't have any empty values
     (let [r (apply hash-map (:recipe p))
           recipe
           (merge
            (when-let [type (:type r)]
              {:recipe/vc (case type
       ...
       (when-not (empty? recipe)
         {:package/recipe recipe}))
     (select-and-rename-keys
      p {:pin :doom.package/pin})
     (when (or (:disable p)
               (:ignore p))
       {:package/deactivated? true}))))
...
(s/def :doom.package/recipe :straight/recipe) ; perhaps minus the straight recipe :name
(s/def :doom.package/disable :elisp/t)
(s/def :doom.package/ignore :elisp/t)
(s/def :doom.package/pin :git/sha)
;; nb: not speccing :modules (for now?)

(s/def :doom/package
  (s/schema [:doom.package/type
             :doom.package/recipe
             :doom.package/disable
             :doom.package/ignore
             :doom.package/pin]))
...
(s/def :my/packages
  (s/and
   ...
   (coll-of :doom/package)
   ;; TODO later: this is Doom-specific, move it out of the main "guix-emacs-generator"
   ;; (remove :doom.package/deactivated?)
   ;; TODO donno if I'm doing this yet
   ;; (remove #(= (:package/name %) "straight"))
   ))

;; (s/conform :my/packages (slurp "/home/pharcosyle/projects/phrenetic/doom-packaging/my-doom-packages.el"))
...
(defkmethods
  :archive/melpa
  (has-recipe [p] (file-exists? (melpa-recipe p)))
  ;; Uncomment once I do `parse-melpa-recipe`
  ;; (get-recipe [p] (parse-melpa-recipe
  ;;                  (edn/read-string
  ;;                   (slurp (melpa-recipe p)))))
  )
...
(def archive-priority [:archive/melpa
                       :archive/elpa])
...
;; TODO if :host is nil then :repo should be a string which is the full URL of the target repository. Handle this.
;; TODO use value of `straight-built-in-pseudo-packages' (let-alist emacs nadvice python) in addition to elpa.scm's `emacs-standard-library?'?
...
;; - what's `straight-recipes-gnu-elpa-ignored-packages' all about?
...
;; TODO do I actually need :include / consider the org-elpa pseudo repository (look at straight-recipes-org-elpa-retrieve in particular)
...
(ns ebullient.home.packaging-wip
  (:require [guile.guix.store :refer [with-store]]
            [guile.guix.git :refer [latest-repository-commit]]))

(defn- download-git-repo [url ref]
  (with-store store
    (latest-repository-commit store url :ref ref)))

;; (println
;;  (download-git-repo "https://github.com/raxod502/straight.el.git" (guile/cons 'branch "develop")))
...
- =man=
  My =clojure-tools= package copies over man pages, Guix's does not.

#+begin_src scheme :tangle (meta-in-dir "clojure.scm")
