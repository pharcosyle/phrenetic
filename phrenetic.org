...
**** Users
#+begin_src scheme
(define* (account #:key name comment shell admin? password (salt "toosimple") bluetooth?)
  (user-account
   (name name)
   (comment (or comment ""))
   (group "users")
  ...
  (package
    ...
    (source
     (origin
       ...
       (patches
        (cons (local-file "wayland-super-fix.patch")
...
******* Patch
#+begin_src diff :tangle (meta-in-dir "wayland-super-fix.patch")
...
(define (he-pipewire svcs)
  (cons*
   (service he-pipewire-service-type)
   (service he-wireplumber-service-type)
   svcs))

(define he-pipewire-service-type
  (service-type
   (name 'he-pipewire)
   (extensions
    (list
...
****** Wireplumber
#+begin_src scheme
(define he-wireplumber-service-type
  (service-type
   (name 'he-wireplumber)
   (extensions
    (list
...
****** Sway
#+begin_src scheme
;; TODO make sure all usages of get-value are gone and I don't require and rde modules
;; TODO some lists of services have conditional entries, make sure falsy ones are filtered out (am I doing this already?)
;; TODO find any arguments with #f and '() defaults and conditionalize them in the usages (persistent-workspaces, timezone...)
;; TODO at end, after trying out waybar: I haven't done all the cleanup for the waybar code, do it.
...
(define* (feature-sway
  ...
  ;; TODO maybe some of the simple services can be combined? The ones in he-sway and he-swayidle, maybe others.
    ...
    (list
      ...
      (home-sway-configuration
       (config
        `((xwayland ,(if xwayland? 'enable 'disable))
          ...
          ;; TODO I think I need to do gexp/ungexp with `dbus'
     ...
     (simple-service
      ...
      `(("xdg-desktop-portal-wlr/config"
         ,(mixed-text-file
           "xdg-desktop-portal-wlr-config"
           ;; TODO do with newlines (note that I should string-join so it's still one arg to `format') (and do it for that sway-screenshot line too)
           ;; TODO then comment this entire simple-service out.
           #~(format #f "[screencast]
output_name=
max_fps=30
...
;; TODO curious what the 303030AA are, wait until I've built the config to check what the finished command looks like. Update: it's a grey color with high opacity I think.
(define (he-sway-screenshot)
...
| .rect | \"\\(.x),\\(.y) \\(.width)x\\(.height)\"' | ~a -b ~a -B ~a"
  ...
  (list
   (simple-service
    'sway-screenshot
    home-sway-service-type
    ;; TODO change these keys? I don't have a Print key (or do I?)
...
(define* (waybar-sway-workspaces
  ...
  (waybar-module
   ...
   `((disable-scroll . #t)
     (format . {icon})
     ;; TODO Andrew Tropin: FIXME: Height becomes higher when icons are not used.
...
(define* (feature-waybar
          ...
          (waybar-modules
           (list
            ;; TODO conditionalize the sway ones?
            (waybar-sway-workspaces)
            ;; (waybar-sway-window)
            (waybar-tray)
            (waybar-idle-inhibitor)
  ...
  (define font-mono
    (and=> #f ; TODO was: (get-value 'font-monospace config)
  ...
  (append
   (list
    (service
     ...
     (home-waybar-configuration
      ...
      (config #(((position . top)
                 (name . main))))
      ;; Andrew Tropin: TODO: fix tray menu styles.
      (style-css
       `(,#~(format #f "@import \"~a\";\n" #$base16-css)
            (*
             ((font-family . #(,@(if font-mono (list font-mono) '())
                               ;; Andrew Tropin: TODO: Add icon-font argument
                               FontAwesome))
   ...
   (list
    (simple-service
     ...
     home-profile-service-type
     (list font-awesome)) ; TODO use my font-awesome package? Or try first with the default guix one (might not reveal anything as long as I have mine installed too though)

    (simple-service
     'waybar-reload-config-on-change
     home-run-on-change-service-type
     ;; TODO factor out the command
...
(define* (he-swayidle #:key
                      sway? ; TODO maybe have this default to true
                      screen-locker
                      (lock-timeout 240)
                      extra-config)
  (list
   ...
   (home-swaylock-configuration
    ...
    (config
     `((show-failed-attempts . #t)
       (daemonize . #t)
       ;; (show-keyboard-layout . ,show-keyboard-layout?) ; TODO
       ;; Andrew Tropin: TODO: Source color from colorscheme
       (color . 3e3e3e)
       (indicator-caps-lock)
       ,@(if extra-config
             extra-config '()))))))
...
;;   (he-sway $ #:screen-locker screen-locker
;;            #:idle-manager swayidle-idle-manager)
;;   (he-swayidle $ #:screen-locker screen-locker))
#+end_src
******* Sway old WIP
#+begin_src scheme
...
#+end_src
** Channel
#+begin_src scheme (meta-in-dir ".guix-channel")
(channel
 (version 0)
 ;; (directory "ebullient") ; TODO might need this
 (dependencies
  (channel
   (name flat)
...
(cons* (channel
        (name 'ebullient) ; TODO rename this? To phrenetic?
    ...
    (list
     ...
     (service home-git-service-type
              (home-git-configuration
               (config
                `((user
                   ((name . "Krzysztof Baranowski") ; FIXME <user-full-name>> ; passwd:gecos could be useful here
