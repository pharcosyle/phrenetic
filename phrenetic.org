...
(def poly-methods (atom {}))

(defn dispatch-method [name k & args]
  (apply (get-in (deref poly-methods) [name k]) args))

(defmacro defpoly [& methods]
  (let [create-var
        (fn [name]
          `(def ~name (partial dispatch-method '~name)))]
    `(do
       ~@(map create-var methods))))

(defmacro defimpl [k & methods]
  (let [register-method
        (fn [[name argslist & body]]
          `(swap! poly-methods assoc-in ['~name ~k] (fn ~argslist ~@body)))]
    `(do
       ~@(map register-method methods))))
