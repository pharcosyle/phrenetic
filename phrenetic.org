...
(s/def :melpa.recipe.files/element
  (s/alt :glob :melpa.recipe.files/glob
         :sublist (s/nest
                   (s/cat :subdirective :melpa.recipe.files/subdirective
                          :element (s/+ :melpa.recipe.files/element)))))
(s/def :melpa.recipe.files/defaults #{:defaults})
(s/def :melpa.recipe/files
  (s/cat
   :defaults (s/? :melpa.recipe.files/defaults)
...
(s/def :melpa.recipe/props
  (s/schema {;; :melpa.recipe/name
             ...
             :url :melpa.recipe.vc/url
             :repo :melpa.recipe.forge/repo
             :commit :melpa.recipe.vc.git/commit
             :branch :melpa.recipe/branch
             :files :melpa.recipe/files}))

(defmulti melpa-recipe-fetcher-type :fetcher)

(defmethod melpa-recipe-fetcher-type 'git [_]
  (s/select :melpa.recipe/props [:url]))
(defmethod melpa-recipe-fetcher-type 'hg [_]
  (s/select :melpa.recipe/props [:url]))
(defmethod melpa-recipe-fetcher-type 'github [_]
  (s/select :melpa.recipe/props [:repo]))
...
(s/def :melpa/recipe
  (s/cat
   ...
   :props (s/*
           (s/and
            :elisp/plist->map
            (s/multi-spec melpa-recipe-fetcher-type :fetcher)))))
...
(s/def :straight.recipe/files
  (s/*
   (s/alt :glob :melpa.recipe.files/glob
          ...
          :defaults :melpa.recipe.files/defaults)))
...
(defn test []
  (let [text (-> my-doom-packages-test
                 slurp
                 edn/read-string)]
    ;; (s/assert (s/coll-of :doom/package) text)
...
(ns ebullient.home.packaging-wip
  (:require [guile :refer [command-line]]
            ...
            [guile.srfi.srfi-11 :refer [let-values]]))
