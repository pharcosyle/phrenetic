...
****** With C source
#+begin_src scheme
(define-public (emacs-with-findable-C-source emacs)
  (package
    (inherit emacs)
    (arguments
...
(ns ebullient.home.packaging-wip
  (:require [clojure.edn :as edn]
            [guile :refer [file-exists?]]))

;; Bug in lokke: if the first argument is `nil` the result is a seq of pairs (two-element vectors) instead of a map.
;; - Changed `reduce1` to `reduce`.
(defn merge
...
(defn multi-fn [var]
  (fn [& args]
    (let [m (meta var)
          val (apply (::dispatch-fn m) args)
          method (get-in m [::dispatch-vals val])]
      (apply method args))))

(defmacro defmulti [name dispatch-fn]
  `(do
     (def ~name nil)
     (def ~name (multi-fn (var ~name)))
     (alter-meta! (var ~name) assoc ::dispatch-fn ~dispatch-fn)))

(defmacro defmethod [name dispatch-val & fn-tail]
  (let [[argslist & body] fn-tail]
    `(alter-meta! (var ~name) assoc-in [::dispatch-vals ~dispatch-val] (fn ~argslist ~@body))))





(defn poly-dispatch [k & _] k)

(defmacro defpoly [& methods]
  (let [create (fn [name]
                 `(defmulti ~name poly-dispatch))]
    `(do
       ~@(map create methods))))

(defmacro defpolymethods [k & methods]
  (let [register
    ...
    (merge
     ...
     (let [type (:type p)]
       (when (some #{type} ['local 'built-in])
         {:doom/type (case type
                       local :doom.type/local
                       built-in :doom.type/built-in)}))
     ...
     ;; TODO at end: ensure an empty map isn't dded / the map doesn't have any empty values
...
;; (println (parse-doom-packages))

(defn parse-deps [form]
  (map (fn [dep]
         (str
          (if (list? dep)
            (first dep)
...
;; TODO here and in parse-single
;; (use-modules (ice-9 regex))

(defn parse-simple-epkg [name dir]
  ["dash"]
  ;; (let [f (str dir "/" name ".el")]
  ;;   (when (file-exists? f)
  ;;     (as-> (slurp f) $
  ;;       (let [rx (make-regexp (str "^" ";*" "\\s*" "Package-Requires:" "\\s*" "(.*)" "$")
  ;;                             regexp/newline)]
  ;;         (regexp-exec rx $))
  ;;       (when $
  ;;         (match:substring $ 1))
...
(defn parse-multifile-epkg [name dir]
  (let [f (str dir "/" name "-pkg.el")]
    (when (file-exists? f)
      (as-> (slurp f) $
        (edn/read-string $)
        (nth $ 4) ; TODO requirements field is optional in emacs package specification, this will error!
        (fnext $) ; Form is wrapped in `quote`.
        (parse-deps $)))))

(defn package-deps [name dir]
  (or (seq (parse-multifile-epkg name dir))
      (parse-simple-epkg name dir)))
...
;;                                       (or (let [type (get p :doom/type)]
;;                                             (some (hash-set type) (vector :doom.type/local :doom.type/built-in)))
